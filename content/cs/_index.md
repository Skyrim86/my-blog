...
# 最后一个子页面被删空时，列表页本身与所有指向它的入口（导航栏、首页）会一起 404。
...
#
# 大类页（/cs/<大类>/）与细分页（/cs/<大类>/<细分>/）**没有**对应的 md 文件：
# 它们由 content/cs/_content.gotmpl（内容适配器）从 data/cs-toolbox.json 现算生成，
# 与数学库同一套机制（见 docs/features.md ㉒）。
title: "CS 库"
layout: "library"
description: "按方向整理的数据结构、算法与工程约定卡：先挑大类，再进细分，一张卡一个页面，正文里写名字就能点开。"
# 这一支不要 RSS：/cs/ 下的页面全是导航页（大类 / 细分），没有正文，
# 喂进 feed 只会多出一串空条目（与 /library/ 同口径）。
outputs: ["HTML"]
---

这里按**大类 → 细分**汇总数据结构、算法与工程实践里的结论卡。每张卡的身份是**类别 + 名字**（卡片左缘的色条与徽章配色就是类别），一张卡一个页面、地址固定，可以直接引用或分享。

页面分三级：这一页是大类（数据结构、算法、工程实践）；点进去是大类下的细分方向；再点进去才是卡片清单。**卡片正文不在这些页面上**——到了细分那一层，点开某一张卡就地展开，正文里的引用也一样。
