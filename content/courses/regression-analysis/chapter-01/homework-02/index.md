...
date: 2026-09-15
draft: false
description: "ILRA 6e 第 2 章推导型与反例型选解：2.25–2.44 与讨论题 2.9（数值计算型不重复录入）。"
# summary 必须写：列表卡片摘要走 `.Summary | plainify`，正文公式已渲染成 HTML+MathML，会被拼成乱码
summary: "ILRA 6e 第 2 章推导型与反例型选解：2.25–2.44 与讨论题 2.9（数值计算型不重复录入）。"
---

> 录入范围：*Introduction to Linear Regression Analysis*（6th ed.，**ILRA 6e**）第 2 章推导型与反例型选解（2.25–2.44 与讨论题 2.9）。数值计算型不录——方法已在正文算例中给出，重复无益。课程作业全解在《作业（一）》。
