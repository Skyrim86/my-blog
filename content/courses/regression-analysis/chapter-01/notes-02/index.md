...
**定理 7.1** 检验 $H_0:\beta_1=\beta_{10}$，显著性水平 $\alpha$，统计量 $t_0$ 由 (7.6) 给出。则
...
**命题 7.6** $p<\alpha\iff$ 在水平 $\alpha$ 下拒绝 $H_0$。

**证明** 双边情形：$p<\alpha\iff P(|t_{n-2}|>|t_{\rm obs}|)<\alpha\iff |t_{\rm obs}|>t_{\alpha/2}(n-2)$，末式即拒绝域（$t$ 分布连续、分位点单调）。单边情形同理。$\square$

**性质 7.7** $p$ 值是"能拒绝 $H_0$ 的最小显著性水平"，因此**越小证据越强**；任何水平下的决策都可以只由 $p$ 作出，不必重新查表。单边检验的 $p$ 值是相应双边 $p$ 值的一半（$t$ 分布对称）。
...
**命题 7.8** 在水平 $\alpha$ 下，双边检验 $H_0:\beta_1=\beta_{10}$ 不拒绝，当且仅当 $\beta_1$ 的 $1-\alpha$ **置信区间**（式 (9.3)）包含 $\beta_{10}$。
...
**定理 9.1** 在 A1–A3 下，$\beta_i$（$i=0,1$）的 $1-\alpha$ 置信区间为
...
**定理 9.4** 在 A1–A3 下，$\sigma^2$ 的 $1-\alpha$ 置信区间为
...
**定义 9.6** 给定 $x=x_0$，$\mu_{y\mid x_0}=E(y\mid x_0)=\beta_0+\beta_1x_0$ 的**点估计**为
...
**定理 9.7** 在 A1–A3 下，$E(\hat\mu_{y\mid x_0})=\mu_{y\mid x_0}$，且
...
**定理 10.2** 在 A1–A3 下，且新观测 $y_0$ 与建模数据**独立**，
...
**定理 10.3** 在 A1–A3 下，$y_0$ 的 $1-\alpha$ **预测区间**为
...
**命题 11.6** 若数据在同一 $x$ 值处有**不同**的 $y$ 观测（重复观测且响应不同），则 $R^2<1$。
