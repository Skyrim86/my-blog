...
title: "数学库"
layout: "library"
description: "按数学分支整理的定理、定义与命题卡：跨课程汇总，一张卡一个页面，正文里写名字就能点开。"
# 卡片标题里带公式（如「与 $t$ 检验的等价性」）的条目由 RenderString 渲染，这一页也要开公式
math: true
---

这里按**大类 → 细分**两级数学分支汇总各门课程里出现的定理、定义与命题。一张卡一个页面，编号稳定，可以直接引用或分享。

卡片正文不在本页：本页只铺索引（大类 → 细分 → 卡片清单），点开才加载那张卡的完整内容——正文里的引用也一样，点一下就地展开。

正文里**直接写结论的名字**（如「由全方差律」「见 Fisher 引理」），读起来就是数学本身；名字是链接，编号（如 1.2、4.4）只作卡片角落的定位小字。
