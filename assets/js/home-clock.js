...
(() => {
    ...
    /* 等级权重：与 data/home-cards.yaml 头部记的那个口径一致（传世 10%、史诗 30%、其余收藏）。
       奇迹在清单里写的是「1 张」而不是百分比，这里把它翻成 1% 的档位权重 —— **这个 1% 是本次
       新增的决定**（不是从清单推出来的）：一年里大约 3~4 天能抽到，比「63 张均匀抽」的 1.6%
       略低，而它仍是全场唯一一张、最难碰到的那张。 */
    const RANK_WEIGHT = [['collector', 60], ['epic', 30], ['legend', 9], ['miracle', 1]];
