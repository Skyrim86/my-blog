...
'      ao *= 1.0 - uShadow * 0.5 * reliefShadow(puv, uL1, hv);',
    ...
    '    }',
    '  } else {',
    '    // 侧面（卡边）：解析法线，材质是纸 / 金属 —— 底色随等级 metal 从纸白走到烫金',
    '    // （2026-09-26，3D 物理 A「侧边随 metal 上色」；「掠射角露出卡厚」半条经判读已达标：',
    '    //    CARD_T 侧墙在掠射角读作实体厚度，未动几何）。纸白 = 与高光那行同一张纸；',
    '    //    烫金 = 固定金 0.7 份额 + 工艺边色 0.3 份额（保证奇迹档读作金、又留工艺身份）。',
    '    //    底色从此是等级的仪表盘：收藏档整圈纸边、奇迹档整圈金属边。',
    '    N = normalize(vec3(cardSdfGrad(vModel.xy), 0.0));',
    '    albedo = mix(vec3(1.0, 0.99, 0.96), mix(vec3(0.86, 0.68, 0.34), uEdge, 0.3), uEdgeMetal);',
    ...
    // ⑤a 渐变环境光（2026-09-26，3D 物理 B）：环境光不再是平的 0.22 —— 上暖下冷的
    //    竖向渐变（棚灯语义：上打暖光、下反冷底）。两端对称取值、mean 逐通道 = 1.0：
    //    正面朝相机无起伏时 N.y = 0 → ambG ≡ 0.22，总光量逐像素不变；渐变只在
    //    浮雕起伏与转动里显形。
    '  vec3 ambG = vec3(0.22) * mix(vec3(0.93, 0.97, 1.06), vec3(1.07, 1.03, 0.94), clamp(N.y * 0.5 + 0.5, 0.0, 1.0));',
    '  vec3 col = albedo * (ambG + 0.26 * ao + 0.55 * ndl1 + 0.17 * ndl2);',
...
'  }',
    ...
    '      // ⑤b 边缘菲涅尔（2026-09-26，3D 物理 B「金属框随角度活一点」）：这一圈的',
    '      //    提亮乘 (1 + 1.2·fres) —— 正视逐像素不变，掠射角圈亮翻倍出头。',
    '      float frim = 1.0 + 1.2 * fres;',
    '      col = mix(col, refr * 1.05, rim * 0.55);',
    '      col += uEdge * rim * rim * 0.20 * frim;',
    '      col += vec3(1.0) * pow(rim, 8.0) * 0.12 * frim;',
    '    }',
    '  }',
  '  // ---------- 透明厚盖：在卡自己的片元里合成 ----------',
