...
4. **低清先上**（`card-3d.js` 的 `setItem`，这条才是主因）：原来只有「xl 到了才换画面」，
   ...
   `faceSeq` 防晚到的图盖住后来居上的那一张；`lowTierOf()` **只认已经 `complete` 的那张**
   （`currentSrc` 一赋值就有值，拿它当低清会跟那条还没调度的 lazy 请求合流）；没 WebGL 时走
   平面大图那条路，同样先低清再升级
...
| ⑤ 收藏库里在弹层里按 › | 1338～2375 ms | 1647～2279 ms（**没改善**） |

线上（真网 + 512 kbit/s 节流、冷点）：③ **66～71 ms**、④ **234 ms**。

⑤ 没改善的原因值得留一笔：那一档要「先上」的低清是**相邻格子的图**（412），而慢网下浏览器还没
把它下下来 —— 低清自己也在等网络，于是这一步省不下任何东西。局部试过三种排法（一起排 / 缓 1.2 s /
页内档立刻+xl 延后），都在 1.6～3.3 s 之间抖，没有一致收益，就停在这里不硬凑了。
