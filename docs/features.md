...
- 动态开着时点壁纸，画面上不会有变化（画布盖着它；而且生成 CSS 把 `--bg-image-*` 置成了 `none`，
  ...
  ③**跨轴不预取、改成意图预热**（第四版，2026-09-22 晚）：第二版为省这一次看不见的下载直接返回，代价是
  关掉动态那一刻现下现解码（最贵的一段，`DECODE_MAX_MS` 600 ms 就是给它留的上限）；第三版改成「动态开
  着时也空闲预取」，切换确实快，但两根轴同时占着下载与解码位图（当前主题两张：浅色 164 KB / 深色 271 KB），
  而这笔流量在访客不点那个按钮时一点用都没有 —— 那正是「两根轴叠在一起」唯一还剩的地方。第四版拆开：
  空闲期只沿当前这条轴（动态开着时零壁纸字节），悬停 / 聚焦 / 按下任一个背景按钮时才跨轴预热
  「这一下要落到的那张」。实测（`check-prefetch.py` 四跑，起点都是动态开着）：
