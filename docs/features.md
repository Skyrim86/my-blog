...
### ㊴ 卡片等级体系（收藏 / 史诗 / 传世 / 奇迹）— `data/home-cards.yaml` 的 `rank` + `21-card-deck.css` + `assets/js/card-3d.js`

每张卡有一个等级，观感按等级递进：**收藏 → 史诗 → 传世 → 奇迹（隐藏）**。`rank` 是**必填字段**，`scripts/check-deck.mjs` 校验值域 —— 漏写它不会报错，只会按「收藏」渲染，而一张本该传世的卡静默降级在页面上完全看不出来（这正是这个仓库反复要拦的那类错）。

**等级与风格是正交的两件事**（等级管材质与边框、风格管卡面上的纹样），所以没有写成「8 风格 × 4 等级 = 32 条规则」，而是延续该文件原有的**变量覆写**体系：等级只声明一组 `--rank-*` 令牌（框宽 `--rank-frame`、整卡覆膜 `--rank-foil`、四角纹章与压印边 `--rank-orn`），风格仍然只改它自己那组。组合爆炸正是这个 CSS 当初膨胀到 67 KB 的原因。

| | 卡面 | 3D 卡边 | 卡背 |
|---|---|---|---|
| 收藏 | 2px（风格自己的框） | 纸白切口 | 现状 |
| 史诗 | 4px 银色金属框 + 40% 虹彩 + 淡内圈 | 烫金（高光变强并被卡边染色） | 徽记加环 + 等级环 |
| 传世 | 5px 暖金框 + 60% 虹彩 + 满纹章 | 黑金 / 镭射（视角驱动的衍射） | 等级专属背面 + 等级带（i18n 词条） |
| 奇迹 | **正面与收藏完全一致** | 光刃（自发光 + 色散） | 徽记背光 + 浮出「奇迹」 |

**「奇迹的正面与收藏一样」不是靠 CSS 藏起来的**：`card-3d.js` 的 `rankOf()` 在显形前**直接返回 collector**，`RANK_3D` 里那套华丽参数根本没被取用 —— 所以从参数层面就看不出来，而不是靠视觉掩盖。

**显形有两条入口**：在弹层里**累计转满 360°**（`turned` 累加拖拽角度），或**在背面停留 1.5 秒**。两条都验过（`../lab/shots/deck3d/assert-rank.js`），并且换到别的卡再换回来会重新藏起来。停留那条**必须用 `setTimeout`**：卡停稳后渲染循环就停了，写在 `update()` 里的判据永远不会执行（traps 第 94 条）。

**首版定级依据是可量化的**：卡面细节量（文件字节 + 画面梯度）+ 源图分辨率，刻意不含「角色人气」这类主观项。分布 63 张里奇迹 1、传世 6、史诗 19、收藏 37。这一版是起点，改 YAML 一行即可。

**3D 侧的「更华丽」三件**：① 卡边材质按等级（纸白 / 烫金 / 镭射 / 光刃）；② **高度场自阴影** —— 沿光方向采 5 个样，被更高的地方挡住就压暗。这一条才是「主体浮起来」的关键：位移只是把画面抬高，而**它把光挡住、在卡面上留一道影**才是眼睛判断「这是一层浮在上面的东西」的依据；凸起内部四周更低，遮挡值自然为 0，所以影只落在背景那侧；③ **由角速度驱动的亮带**（转得快就亮、停下就淡掉；常驻会像印上去的）与一层错频虹彩。
...
**今日一卡**：从**卡片组的清单**里按等级加权抽一张，展示缩略图 + 名字 + 系列·等级；
点一下开那张卡的三维弹层（左栏卡片组同时翻过去）。
- **数据不另抄一份**：读同页 `.home-deck` 的 `data-deck`（63 张的图 URL、名字、系列、等级都在里面，
  约 13KB）。代价是耦合：卡片组那块被摘掉时今日一卡整块不出现（符合「宁可没有，也不给一个错的」）。
...
4. **名字写卡下，不压在卡上**。首页那套 `.home-card-plate` 是绝对定位浮层、自带 `backdrop-filter: blur(6px)`：一页 63 个浮层就是 63 层背景模糊，而且格子只有 ~205px 宽，浮层会盖掉小半张画。改成卡下说明（`.deck-tile-cap`：名字 + 系列 · 等级）。等级另有一层 `.home-card-rank` 覆膜照留 —— 那是这小格子上唯一的等级**观感**（覆膜 + 四角纹章），文字那行是**信息**，两者不重复。
