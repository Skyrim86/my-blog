...
- **空闲预取另一套**：加载完成、浏览器空闲且**页面在前台**时，把「另一套 + 当前主题」那一张用 `new Image()` 取回来并 `decode()` —— 点切换时图已在解码缓存里，是秒切。**动态背景开着时改取两张**（当前那张 + 另一套）：关掉动态要落到的就是「当前那张」，它此刻在 CSS 里被摘掉了、不预取就要现下现解码（见本节的「两个按钮」一段与 `check-prefetch.py` 的 4 跑对照）。跳过三种情况：`navigator.connection.saveData`、`effectiveType` 是 2g/3g（桌面浏览器基本不报这个值，尽力而为）、以及页面在后台（改为 `visibilitychange` 回到前台时再补）。明暗按钮改的是 `data-theme`，所以用 `MutationObserver` 盯着它——换了主题，「另一套那一张」就换成另一张图了。
  - 图片来源是**模板给**的：`extend_head.html` 把每套两个主题的指纹 URL 一并写进现有的 `data-presets` JSON（每页多约 110 字节/套）。脚本不自己拼 URL —— 指纹在构建期算，JS 拼不出来。
  - **代价要认清**：从不切换背景的访客也会多下一张（**日间 63 KB / 夜间 128 KB**，对照：默认套自己那张是 93 KB / 137 KB）；动态背景开着时是两张（当前 + 另一套，当前主题），实测 **265 KB** —— 换来的是关掉动态那一刻不用等（见 §「两个按钮」与 `check-prefetch.py`）。只在空闲且可见时发生，所以不推迟 `load`；实测 4G 首页传输因此从约 232 KB 升到 295 KB（`../lab/结果/home/perf-home-4g*.json`）。想收可以改两处：只对非触屏预取、或把 `onIdle` 的延迟拉长。
- **切换做交叉淡入，而不是硬切**：`background-image` 是离散属性、过渡不插值，所以做法是——点击时把**当前 `body::before` 的已解析背景栈**（`getComputedStyle(body, '::before')` 的 `backgroundImage/Size/Position/Repeat`，蒙版渐变与图片 URL 都在里面）快照到一个临时 `<div class="bg-ghost">` 上，等目标图 `decode()` 就绪后改 `data-bg`，再让 ghost 从 1 淡到 0（250ms，样式在 `00-theme.css`）。**不在 CSS 里抄第二份背景栈**：抄的那份迟早与 `::before` 漂移。
  - 层级：ghost 是 `body` 的子节点、与 `body::before` 同为 `z-index: -1`，按树序绘制 ⇒ 它画在**旧壁纸之上、玻璃质感层（`body::after`）之下**，所以淡出期间质感层不动、只有壁纸在换。实测的中段帧见 `../lab/结果/bgfade/vis-1.png`（新旧两张叠在一起，正文与导航清晰压在画面上）。
...
- **试过 preload 当前那张图，结论是「不采纳」**（2026-09-19）：给「当前套 + 当前主题」那张发 `<link rel=preload as=image>` 确实能让请求**确定性提前约 240 ms**（4G 实测：请求起点 343/345/343/333 → 112/90/102/101 ms，四跑一致），但**壁纸真正到货的时间没变** —— 慢链路上带宽才是瓶颈，提前开始的代价是下载被同期的 HTML/CSS 挤长（base 343+346 ms ≈ preload 90+601 ms），而 FCP 那点差别完全落在同一台机器 ±150 ms 的轮间噪声里（base FCP 348~572、preload 340~524）。把 preload 放到 `<head>` **开头**还会和渲染关键资源抢队首，出现 DCL 430 → 584 ms 的反效果（放到样式表之后才平）。
  ...
  - 证据：`../lab/结果/home/perf-preload-ab.json`（head 与 tail 两种摆放各一次）与 `../lab/结果/home/perf-preload-rep.json`（tail 摆放重复一对，逐次数字在内）。
...
**24px 的代价量过，可以忽略**：`../lab/工具/scrollcost.py` 在同一页面对 `blur(3px)` 与 `blur(24px)` 做 A/B —— 帧间隔中位都是 **6.06 ms**、`DirectRenderer::DrawFrame` 各约 39 ms、`LocalFrameView::RunPaintLifecycle` 18.5 → 19.2 ms（约 1.1 秒滚动窗口内属噪声）、掉帧计数相同。Chromium 的 backdrop 模糊代价主要来自「建立 backdrop root」，与半径关系不大。
...
**局部玻璃底衬**（`00-theme.css` 的「直接压在壁纸上的文字」一节）：清单是**量出来的**，不是猜的 —— `../lab/结果/girlbg/measure.py` 遍历页面上每个可见文字元素，凡祖先链里没有面板/卡片的都在表里：
...
**对比度是量过的，改图或改蒙版都要重量**：做法是在页面里遍历**可见**文字元素，取它自己的算色，沿祖先链把 `background-color` 逐层合成到 `body::before` 的底色上（底色 = 蒙版 rgba 按视口 y 插值后，盖在实际背景图像素上——背景是 `position: fixed`，所以蒙版只取决于视口 y，与滚动无关），再算 `(L1+0.05)/(L2+0.05)`。两个容易做错的细节：**合成祖先链时要排除 `html`**（它的底色是被 `body::before` 盖住的画布背景，算进去会得出「背景永远不透明」的错结论），**底色要按 `cover` 几何用 canvas 取真实像素**而不是取图片平均色。可复跑的脚本在仓库外：`../lab/结果/girlbg/contrast.js`（页内测量）+ `measure.py`（按主题 × 套 × 页型驱动）。**解析器只认 `rgba()`/`rgb()` 与 `color(srgb …)`**，所以底衬这类色值要写成 `rgba()` 字面量，别用 `color-mix()`（理由见下面「局部底衬」）。
...
**动态背景（WebGL shader）—— 2026-09-22 落地，当天晚上拆成「与静态壁纸并列的另一根轴」并加了第二套**

它不是 presets 里的一「套」：`[[params.appearance.shaders]]` 是**独立的一份清单**，页面上两个按钮、两份 localStorage。
拆开的原因很直接 —— 原先动态背景与壁纸挤在同一个循环按钮上、共用一份偏好，想固定看某张壁纸又想偶尔开一下动态，
得点着循环数圈，而且数到哪一套完全看上次停在哪。

**两个按钮的分工**（都在 `assets/js/bg-switch.js` 里，一个文件接线两个按钮：数据同源、交叉淡入只有一份实现）：
...
- 动态开着时点壁纸，画面上不会有变化（画布盖着它；而且生成 CSS 把 `--bg-image-*` 置成了 `none`，
  ...
  ③动态开着时**照样把壁纸预取好**（第三版改的）：③缺了的话关掉动态那一刻要现下现解码，而那正是
  切换里最贵的一段（`DECODE_MAX_MS` 600 ms 就是给它留的上限）。预取让那一刻只花交叉淡入那 ~250 ms
  的地板。实测（`check-prefetch.py`，四跑：快网/慢网 × 预取开/无预取）：
  线上 **284 ms vs 741 ms**（拦掉预取时那张图要下 465 ms）、512 kbit/s 下 **279 ms vs 875 ms**（没预取时
  顶到 600 ms 上限才切，图 2.8 s 才到）；代价 ~265 KB（当前主题两张：当前 + 下一套）；
...
动态套的 `id` 一律形如 `__dyn-xxx`，于是：

- 生成的 CSS 用**一条** `:root[data-bg^="__dyn-"]{--bg-image-*:none}` 覆盖全部动态套（加第二套不必再动模板或 CSS）；
- 首屏内联脚本与切换脚本都按这个前缀分辨「静态 / 动态」两侧；
- `presets` 的 id 规则是 `^[a-z0-9-]+$`，下划线开头进不去那个命名空间，两边永不撞。

切换脚本**只改 `<html data-bg>`**，跑不跑由 `bg-shader.js` 读属性自己决定（`window.__bg.refresh()`，不猜按钮意图）；
程序按 `kind|oct` 缓存，切套只在换画法或改 `oct` 时重新编译。现有两套：

- **极光**（抽象）：极光带 + 星野；浅色主题是白昼天光（下面「浅色那套的两个做法」讲的就是它）。
- **夜樱**（二次元，2026-09-22 晚加）：平滑夜空渐变 + 地平线天光 + 月（月海、边缘压暗、月晕，**同时是整幅的光方向**）
  + 两层硬边云（云体两档 + 朝月侧边缘一圈亮）+ 星（格内抖动，被云挡住）+ 16 片带缺口的花瓣（翻面时压扁）+ 三层远山剪影。
  观感是**逐版看图改出来的**（v1 一片紫雾 → v2 色带量化出水平硬缝 → v3 花瓣像两圆并排的药丸 → 现在这版），
  返工记录见 [`exp-bg.md`](exp-bg.md)。两条设计约束：**浅色主题必须亮**（深色字，天空与远山都停在 luma≥0.70）、
  **亮盘不放中间**（月亮固定在左上角 —— 正文栏在 `uv.x .2~.8`，正文矩形里出现一块高亮度像素才是对比度的真风险面）。

**首屏那段内联脚本的四条规则**（顺序即优先级，两个键在这里合成一个 `data-bg`）：

1. 默认 = 静态默认套；
2. 存过的**静态**壁纸合法就用它；
3. `pref-bg-dyn` 有值且合法 = 动态开着（盖过 ②）；
4. `dynamicDefault` 只在**两个键都没存过**时生效 —— 他在静态侧做过选择，说明他要看壁纸。另外 `prefers-reduced-motion`
   下不套用 ④：那套 shader 根本不会挂，套上去只会白等一次属性回退。

**顺带修掉一个老问题**：原先 `shader.default = true` 会把 `:root` 那一块也变成「无图」，于是禁 JS 的访客（含爬虫）
根本没有背景；现在 `:root` 恒等于静态默认套，动态只由那段内联脚本（需要 JS）加身。

**配置走 `data-opts` 属性，不走内联 `window.__bgOpts`**：在 `<script>` 上下文里 html/template 会把 `jsonify` 的结果
再转义一次，出来是**字符串**而不是对象 —— `SETS[id]` 恒 undefined，整套动态背景静默不挂、构建也不报错（踩过，见
[`traps.md`](traps.md)）。属性值由 HTML 解析器解码，没有这层转义；`bg-shader.js` 用 `document.currentScript.dataset.opts` 读。

**交叉淡入有两个方向**（`background-image` 是离散属性、过渡不插值，只能叠 ghost 层；样式在 `00-theme.css`）：

- 当前是**静态套** → ghost 快照**当前整个背景栈**（蒙版 + 旧图），改完属性让它淡出；
- 当前是**动态背景**（`body::before` 里没有图可快照）→ 目标壁纸先在 ghost 上从 0 淡到 1 盖住画布，落定后
  **同一帧里**把 ghost 的背景换成新套的已解析栈（含蒙版）再撤掉 —— 两者像素等价，接缝为 0；
- 动态套之间切换没有图可淡，只把画布自己压 60 ms。
...
4× 像素（50 Mpx）时帧间隔崩到 max 115 ms，而 shader 本身只有 1.37 ms。重绘上限 30 fps（同一窗口 `draw` 367 → 61，
帧间隔量不出差别）、页面不可见停 rAF、弹层期间由 `home-deck.js` 停背景 rAF（与卡的调速器没有协调，弹层期间把预算全留给卡）。
夜樱套的结构比极光重（4 次 fbm + 16 片花瓣 + 星），但**本机在 headless 里拿不到 GPU 计时器**（`stats().gpu = {n:0}`），
这一项**没有数字**；按像素预算它与极光同量级，有真机复核再补。

**对比度是逐帧量的**（1280×800 @dpr2 首页，6~8 帧取最坏，`live.py --frames` + `analyze-contrast.py`）：

| 动态套 | 深色主题 | 浅色主题 | 元素数 | 不合格（AA 4.5） |
|---|---|---|---|---|
| 极光 | 6.43 | 5.84 | 67 | 0 |
| 夜樱 | 6.01 | 5.93 | 67~69 | 0 |

浅色主题是风险面（深字 + 会变暗的底）：只有一套深色配色时浅色主题有 28 项掉到 4.30~4.49，所以两套都按 `data-theme`
切配色。**注意文字并不直接坐在背景上**（正文都在 0.8 不透明度的玻璃面板里），背景的亮度变化被面板挡掉了大半 ——
这也意味着「背景能画多明显」的约束不在文字，而在观感协调。

**极光浅色那套的两个做法**（2026-09-22 晚：第一版「看不出来」之后的返工）：① 光带色做**亮度中性提饱和**
`c' = luma + (c-luma)*k`（k=2.3）—— WCAG 只看亮度，这个变换在数学上保持亮度不变，所以彩度可以放开提而对比度一分不掉
（6.12 → 5.72 那 0.40 全部来自「底色压暗一档」，与提饱和无关）；② 上色方式从「把底压暗一层」改成「按 glow 在底与光带色之间插值」
—— 压暗得到的是灰雾，插值得到的才是带颜色的光带。

**分析脚本的判据 2026-09-22 晚改过一次**（`analyze-contrast.py`）：底色众数**先把笔画像素剔掉**再取。短而密的 CJK 标题
（「最近更新」四个字）笔画能占矩形面积 30%+，不剔的话众数就是文字色本身 → 对比度恒等 1.00，看着像不合格；而它只在
「背景与文字色的量化桶相邻」时才翻车 —— 换一套背景就换一个结论（夜樱那版正是因此多出一个假阳性，而极光那版报绿）。
...
**几何量必须缓存**：正文与每个目录项的绝对偏移只在「重新测量」时算一次，滚动路径上只做 `window.scrollY` 的算术。原来的写法每次 scroll 都要对正文调 `getBoundingClientRect()` 与 `offsetHeight`、再对每个目录项逐个取 rect，而最重的公式页有 1082 KB HTML / 1.68 万个 `<span>`。用 CDP 的 Performance 计数器量（`../lab/工具/jank.py`，110 次滚动）：旧写法 `ScriptDuration` 0.019~0.020 s，缓存后 0.004~0.005 s，**滚动脚本开销降到 1/4**。失效时机是 `resize` / `load` / `document.fonts.ready` / `ResizeObserver(.post-single)`，最后一条是为了兜住「图片或字体迟到导致正文高度变了」。
...
- **右位**：与站点图标同源的少女 `assets/images/mascot.webp`（站点图标那份白底素材 → `tools/backgrounds/make-mascot.py` 抠图，40 KB，366×720）。**2026-09-18 重做了抠图**，因为旧版在深色主题下沿整个轮廓留了一圈白边、脖子与马尾之间还留着一块白斑（原图白色背景没删干净）。白底图有三个坑，三个都得处理：
  ...
  另外填掉人物内部 <32px 的纯白噪点（24 处），否则领子上会留透光的针眼。脚本把「深色 / 浅色各一份」的预览写到仓库外的 `../lab/结果/mascot_check.png`，**改完必须肉眼看一次那两个主题**（白边在深色主题下最明显）。出了图别忘同步 `extend_footer.html` 里 `<img>` 的 `width`/`height`（脚本会打印出图尺寸——按 `height` 反推宽度会差 1px）。
...
- **首页每天只轮这 12 张**（2026-09-21 用户要求：「首页展示卡片不用全部，每天随机从库中抽 12 张就行」）：模板声明 `data-deck-daily="12"`，JS 以**「年 × 1000 + 一年中的第几天」（访客本地日期）**当种子，从 63 张里抽 12 张当这一天的展示集 —— 同一天所有访客看到同一批、换日就换批、**不需要每日重构建**（静态站没有「今天」，这是唯一不依赖每日 CI 的做法）。序号因此写成 `01 / 12`。抽出的 12 张**按清单原序排回**，与收藏库里那一排的顺序对得上。
  ...
  - 算法验过（`../lab/工具/deck-daily-test.js`，从源码里抽出真函数跑）：同一天两次一致、30 天 30 种组合、365 天里 63 张全部露过面（次数 50~84、均 70）、n = 清单长度时退化成整副。
...
- **一副牌五个系列**（63 张）：
  ...
  - 卡夫卡那一系**每条查询都带 `rating:safe`**：站点上她的 `solo simple_background` 不带这个过滤有 100 条、带上只剩 2 条，那些是 questionable 分级，靠 tags 里的敏感词滤不干净 —— 而这是要放在公开首页的图。代价是池子薄：全站 safe 单人只有二十来条、能用的七八条，所以这一系只到 7 张（另有一张画面里带作者署名 `©GOODGOODSPHERE`，否掉了、源图也删了）。挑图口径与接触表在 `../lab/结果/revamp/pick-deck.py`（打分：透明底 +120 / 简单底 +30 / solo +25 / 多人 −45 / 签名文字 −12），下载归一化在 `tools/cards/fetch-sources.py`。
- **交互**（都在 `assets/js/home-deck.js`；脚本在首页与收藏库加载 —— 后者页内没有轮播卡，只用它的弹层与 3D 查看器，见 ㊿）：
  ...
  - **换卡要等多久（2026-09-22 晚重做，`../lab/结果/deck-perf/check-cardflow.py`）**：这一条从头到尾只讲一件事 —— **别让「按下去了」与「画面换人」之间隔着一次下载**。三处改动：
    - **页内预取的档位要按 DPR 挑**：`srcset` 写的是 `s 272w … l 544w`，2x 屏才走 `l`。此前无脑预取 `l`，于是在 1x 屏上那次预取等于白下（浏览器要的是 `s`）。现在按 `devicePixelRatio` 选，预取完再 `decode()`（下载完不解码，切换那一帧仍要现解码）。
    - **弹层里预取的是「弹层要用的那两张」**：页内那一档和弹层里那张 760（`xl`）+ 深度图不是一回事（每张约 64 KB）。弹层打开时按「这一张先、后两张随后」的顺序排队 —— 四张一起排会把打开弹层从 2.2 s 拖到 3.2 s（本地是 HTTP/1.1 单连接，`fetchPriority: low` 不改变排队顺序）。
    - **意图预取**：指针停在首页的 ⤢ 或收藏库某一格上 **200 ms** 才触发（悬停即取会让匆匆划过的指针白下 60 KB；一进页面就为 63 张各下 64 KB 更是灾难）。键盘用户走 `focusin` 同一条路。
    - **低清先上（这条才是主因）**：`card-3d.js` 原来只有「xl 那一张贴图到了才换画面」，所以等多久取决于 760 那一档下多久。现在先上传**页内那张真正下过的档**（由 `home-deck.js` 的 `lowTierOf()` 从 `<img>.currentSrc` 读，不猜档），它必然在缓存里，几乎立刻换上；xl 到了再换一次，观感是「先变过来、再变锐」。两个坑都踩过：**只认已经 `complete` 的那张**（`currentSrc` 在 `src` 一赋值就有值，拿它当低清会让这次上传跟那条还没调度的 lazy 请求合流，等的是浏览器的懒加载排队）；**档位按页定、不是按卡**（首页卡是 `l`/`s`，收藏库格子是 `t`=412 —— `pageTierOf()` 直接问墙上那一格的 `<img>`）。没 WebGL 的机器走平面大图那条路，同样先摆低清再升级。`faceSeq` 防晚到的图盖住后来居上的那一张。
    - 实测（本地站点 + 512 kbit/s 节流、冷缓存、DPR2；判据是 `card3d.stats().faceSwaps` 自增 = **画面真的换人了**，不是某张图下完）：

      | 档位 | 改前 | 改后 |
      |---|---|---|
      | 弹层里按 › 翻卡（首页） | 1453～2935 ms | **18～89 ms** |
      | 首页 ⤢ 开弹层 | 2594～3545 ms | **216～233 ms** |
      | 收藏库点开一格 | 3065～9238 ms | **45～239 ms** |
      | 页内换下一张（**DPR1**） | 437～1040 ms | **0～20 ms** |
      | 收藏库里在弹层里按 › | 1338～2375 ms | 1647～2279 ms（**这一档没改善**） |

      最后一档的老实话：那儿要低清先上的那张是**相邻格子的图**，慢网下浏览器还没下到它，于是「先上低清」这一步无从谈起 —— 低清自己也要等网络。它在本地服务上尤其难看（HTTP/1.0 + `Connection: close`，每条请求新建连接），但**没有实测证据说线上就好了**，记在 `docs/pending.md` 里等以后量。

      改前的数字取的是「xl 那张下完」，改后取的是贴图换人 —— 同一件事的两种量法，因为在旧代码里它们本来就是同一时刻。没 WebGL 的机器走平面大图那条路，同一套「低清先上」单独验过（`../lab/结果/deck-perf/check-fallback.py`，`--disable-webgl`：低清 6 ms 落地、xl 1933 ms 升级）。壁纸那一轴的同类改动见 ⑫ 的「空闲预取」一段（那边量的是 `check-prefetch.py`）。**注意本地服务的口径**：它发 HTTP/1.0 + `Connection: close` 且没有 `Cache-Control`，每条请求都得新建连接，线上（HTTP/2 + 指纹文件名 + 强缓存）只会更快。
...
- **十六种卡面风格，全是 CSS**（换风格只改 YAML 的 `style` 字段，不重出图）：`foil` 全息（软边虹彩带 + 光栅颗粒 + 悬停时带位平移）· `gold` 金边（拉丝金属框 + 斜向柔光 + 描金角）· `washi` 和纸（**各向异性**纸纤维 + 纸浆云斑）· `frost` 霜蓝（收尖的软扇骨：6 主枝 + 12 短枝）· `ink` 墨 · `glass` 玻璃（**四件套**：倒角 + 柔和高光 + 底边折射亮线 + **两级假厚度**（`.home-card-lens` 与它的 `::before` 各折射一档、`::after` 给一圈青/品红的边缘色散），外加一道「窗玻璃反光」，磨砂只做下半张）· `gothic` 哥特教堂玻璃（手抖过的铅条 + 四块宝石片 + 暗角）· `sakura` 樱纹（五片**真花瓣轮廓**）· `asanoha` 麻叶纹（软笔触的三向格 + 手抖）· `seigaiha` 青海波（渐隐的半圆环）· `yukika` 雪华（**散落的九朵**带分枝六角雪花：大小 0.6~1.7 倍、角度与浓淡都不同，`--cmask-size: 100% 100%` 是整卡贴花**不是平铺**）· `nishiki` 织锦（绸面柔光 + 纤维 + 斜纹暗示）· `kintsugi` 金继（黑底 + 4~5 道**弯曲分叉**的金缝，缝的**暗侧**单独贴在 `::after` 上用 multiply 压暗 → 一侧受光一侧落影的厚度感；釉面反光借装饰层）· `uchiwa` 团扇（14 根**弓形**扇骨 + 扇箍 + 纸面透光）· **`holo-prism` 星芒全息**（放射虹彩 + 星点，悬停时整层转一点）· **`holo-gold` 金箔全息**（暖金衍射带 + 镜面窄反光）。另有 `pixel` 备着（当前没有卡用它；生成器的 `pixel` 通道与样式都留着，它的硬边是题材本身、不挂手抖滤镜）。
  ...
  **2026-09-20 晚：第二版重做，主题是「不要生硬的直线、玻璃要有立体感」。** 第一版的问题量出来了：织锦一张卡上约 **394 条 1px 硬直线**（1px/4px 三向网格）、麻叶纹 ≈97、和纸 ≈89、全息 ≈89 条 4px 硬边彩虹条；金继是三根笔直金线；玻璃只有「一道斜渐变 + 1px 内环 + 半张磨砂」。这一版换掉的技法：
  ...
  - 验收口径也加了一条：**混合模式要挑在浅色与深色两种画作上都读得出来** —— `screen` 压在浅画上等于没有（金继第一版的金色裂缝在「木刀」那张浅画上完全看不见）、`multiply` 压在深画上同理。对比图因此改成「同一批风格 × 浅色画 + 深色画 × 明暗主题」四组（`../lab/结果/deck2/styles-final-v2-{light,dark}.png`）。
  ...
  - 十四种风格的对比图（同一张卡、明暗两套）：`../lab/结果/deck2/styles-final-{light,dark}.png`。
- **流水线**（改卡片只碰左上角那一个文件）：
  ```
  data/home-cards.yaml  ← 单一事实源（图片名/源图/取景/名字/风格/出处）
     ├─ tools/cards/make-cards.py     出卡面 600×840 + 接触表 ../lab/结果/cards_check.png + 报孤儿产物
...
**为什么是 lossless 而不是常规的 q82**：调色板 PNG（`03_resid.png`、`04_obs_vs_fit.png`）走有损 WebP 会**涨一倍** —— 实测 q82 +100.4%、q90 +150.2%、最大像素差 138；而无损四张全部更小（−39.4% ~ −74.1%）。总账 49598 → 17176 B（**−65.4%**）。**产物与源 PNG 逐像素完全一致**：`Resize "<W>x<H> webp lossless"` 是纯格式转换、不做重采样，已用 `../lab/工具/verify_shipped_webp.py` 对 `public/` 里真正发出的那 4 个文件逐个 `numpy` 比对确认（不是「看着差不多」）。量法脚本 `../lab/工具/imgfmt_measure.py`（各档字节 + PSNR）与 `../lab/工具/hugo_webp_verify.py`（草稿期像素比对）可复跑。
...
**实测**（1440×950、`--force-device-scale-factor=1`，证据在 `../lab/结果/deck3d/`）：拖动 14 帧的覆盖率最低 9.3%（**没有空帧**，判据是 α>128 的像素占比而不是「截图看着不黑」）；松手精确停在 -π（背面），翻面按钮两个方向都对；关弹层后帧数增量为 0；控制台 0 错误 0 警告。角度对照 `angles-light.png`（0/26/46/63/90/132/166/180°，90° 时覆盖率 3.8% 就是那条真实的卡边）、明暗两套 `open-sw-light.png` / `open-gpu-dark.png`（后者在真显卡上拍，与软渲染的读数几乎一致）。
...
**先做了 die-cut 的样张（结论：不做）**。`lab/结果/deck7/diecut.py` 用现有深度图当蒙版，
...
`lab/结果/deck7/depth_model_ab.py`（出表 + 边缘 3× 放大图）：
...
**A/B 实测**（同一角度、同一张 legend 卡，`lab/工具/shot3d.py` 按住拖到 60° 掠射角再截图）：
...
- `card-3d.js` 是 `defer`（低优先级、gzip 后约 14 KB），首屏并不等它；改成按需懒加载只会让
  **第一次点开弹层变慢**，净收益为负 —— 不做。`prefetch()` 也早就在 `home-deck.js` 里预取后两张的 2x 图了。
  （2026-09-22 晚补充：预取那条已按下面 §「换卡要等多久」重写 —— 页内那一档要**按 DPR 挑**，
  另外补了「弹层里预取 xl + 深度图」与「悬停/聚焦 200 ms 才触发的意图预取」。）
...
**奇迹仍然「正面与收藏完全一致」**，这条不变量不许破：`card-3d.js` 的 `rankOf()` 在显形前**直接返回 collector**，`RANK_3D` 里那套华丽参数根本没被取用；令牌那一侧也是一样 —— `.home-card-rank--miracle` 故意照抄收藏那一套，只有 `.is-revealed` 才切到另一组。`lab/结果/deck6/probe.js` 里有一条断言专门比这六个令牌值（显形前的奇迹 vs 收藏），六档令牌阶梯里也含它。
...
守卫⑤ 报的那条提示就是它们）。看一眼它们长什么样的工具是 `../lab/结果/ornaments/build.py`（生成一页对照，
...
- `python scripts/rank-deck.py` 打印六档名单（带三项子分）、写 `data/rank-scores.json`、出接触表 `../lab/结果/rank-pick/sheet.png`
...
- 实测（1680×960 首页，壁纸区域平均亮度）：左侧 118.8 → 123.6（**+4.0%**）、
  ...
  是因为蒙版与玻璃板两层把它稀释掉了。
- 对比度（`../lab/结果/girlbg/measure.py`，改后重跑）：**最紧处 light/girl/home 4.88 → 4.89**，
...
**八、白纱蒙版放薄（「背景还是蒙蒙的」那条）**。逐层剥开量过（`../lab/结果/revamp2/cmp-layers.png`）：
...
- 改这两个数必须重跑 `../lab/结果/girlbg/measure.py both`（做法：把脚本里的 1313 换成 8791）。
...
- **字体**：Playfair Display 600（SIL OFL 1.1，许可全文在 `assets/fonts/rose-clock.LICENSE.txt`）。
  ...
  手写花体（Italianno）也做过对比图（`../lab/结果/revamp2/cmp-fonts.png`，六版并排：
...
（`../lab/结果/clock-blank/`：A 留空 / B 三条轴 / C 打卡月历 / D 四格统计 / E 月历+今日轴，
...
- **权重 60/30/9/1**（收藏/史诗/传世/奇迹），与 `data/home-cards.yaml` 头部记的口径一致
  ...
  复算脚本 `../lab/结果/clock-blank/pick-days.mjs`（直接读产物的 data-deck，与页面同一份数据）。
...
**验证与证据**（脚本与产物都在 `../lab/结果/clock-blank/`）：
...
- 对比度：`../lab/结果/clock-blank/contrast-8791.py both`（**复用** `girlbg/measure.py` 的 JS 与口径，
...
这两项是**先出对比图、用户圈定后**才落地的（图在 `../lab/结果/beautify/frost/`）。
...
量法：`../lab/工具/perf.py`（自管 Edge headless + CDP，`--throttle 4g` 按 4 Mbps/70 ms 模拟）与 `../lab/工具/jank.py`（滚动期间读 CDP Performance 计数器）。**下面每个数字都要能复跑**，改完外观/资源后重跑一次对账。
...
同一套量法复跑（生产构建 + `../lab/工具/serve_public.py` 起静态副本，**不用 dev server** —— 它会多下 78.6 KB 的 `livereload.js`）：

| 指标 | 值 | 出处 |
|---|---|---|
| 首页 4G 传输 / 请求数 | 295.5 KB / 21 | `../lab/结果/home/perf-home-4g2.json` |
| 首页 4G DCL / load / FCP | 441 / 742 / 564 ms | 同上（两次跑 DCL 424~441、FCP 496~564，落在既有噪声里） |
| 首页 HTML / DOM | 16 KB / 230 节点（原 17 KB / 168） | `../lab/结果/home/*.png` 同批构建 + `startjank.json` |
| layout-shift / 长任务 | **0 条 / 0 条**（六轮，开/关动画都一样） | `../lab/结果/home/startjank-anim-ab.json`（交替重复三轮 ×2 臂；2026-09-22 从系统临时区 `/tmp/labrun` 迁入，原指针已作废） |
...
量法：`../lab/工具/perf.py --throttle 4g --out baseline-m1.json`（生产构建 + `serve_public.py 8791`）与新探针 `../lab/工具/csscover.py`（CDP `CSS.startRuleUsageTracking`）。**本地静态副本不发 gzip**，所以下表传输值只看结构与时间线，别当字节账读。
...
**观感**：对照图 `../lab/结果/cards-412-vs-544/sheet-412-vs-544.jpg`（左 412 / 右 544，按 DPR2 真实显示的 411 物理像素并排，右半带 3× 放大局部）。逐像素差 3.4~5.7 / 255，全部来自 544 档被缩到 411 的重采样 —— DPR2 屏上 412 档是 1:1 映射。
...
线上量到的病：`/library/` 单页铺 80 张索引卡 + 分支大纲，线上 4 G 实测（`../lab/工具/startjank.py --arms libperf-arms-live.json`）DCL 494 ms / load 1405 ms / 加载期一个 **103 ms** 长任务 / 滚动区 6588 px，而它承担的信息只是「有哪些大类」。

改法与拆法见 ㉒。**前后对账用同一个方法量**（真窗口 + CDP 4 G 节流，`startjank.py --arms ../lab/结果/libperf-before-after.json`）：改前那份不是旧数据，是用 `git worktree add --detach D:/projects/blog/lab/结果/before-lib HEAD` 把已发布的站点单独构建、另起一个 `serve_public.py` 量出来的（本机 TTFB 两边都是 4 ms，可直接比）。
...
- **KaTeX 字形预加载**（`<link rel=preload as=font>` 按本页出现的类名挑字形）：本地延迟模型（`../lab/工具/serve_delay.py`，每请求 +300 ms，HTTP/1.1）下**无效**——首个字体请求确实从 686 ms 提前到 333 ms，但最后一个字体到达时间不变（1265 → 1252 ms），FCP 反而从 802 退到 932 ms（4 次重复，离散 ±10 ms）。原因是浏览器对单主机只有 6 条连接，9 个字形（133 KB）一起挤进去，把阻断首屏的 CSS 往后排。生产的 Fastly 走 HTTP/2 多路复用，不会再排队，**但线上没有实测，所以没合并**（真要试：合并后跑一次 `../lab/工具/startjank.py --arms <线上 arms>` 对 FCP 与字体到达时间，不达标就撤）。
...
| 最长长任务（headless，warm） | 97 ms | **64 ms**（模型估 60~100，见 `../lab/记录/q3-split-plan.md`） | 52 ms |
| 该页排序 | 全站第 1 重 | 第 8 重 | — |
...
- KaTeX 在公式页**按需**加载 woff2 字形（`katex.min.css` 里 20 个 `@font-face`，浏览器只取页面真正用到的那几个）：M1 笔记页最多见 9 个共 **133 KB**（`Math-Italic`、`Main-Bold`、`Math-BoldItalic`、`Caligraphic`、`AMS`、`Size1~3` 等），`katex.min.css` 23 KB。早期记的「6 个 107 KB」是 `startjank.py` 的 resources 列表被截断后的低估，准数请用 `../lab/工具/fonttruth.py` 量 `document.fonts`。只在真有公式的页面加载（`extend_head.html` 的三条件判据）。要再降只能做字体子集化，收益不确定、维护成本高。
...
稳态早就够了：真窗口、165 Hz（帧预算 6.06 ms）下持续滚动，3 s 之后 p99 6.2–6.3 ms、max 6.3 ms、**没有一帧超过 7 ms**。掉帧全部集中在打开页面后的 1–3 s。量法 `../lab/工具/startjank.py`（`addScriptToEvaluateOnNewDocument` 注入到 document 起点，导航后立刻逐帧滚动，同时收 longtask / layout-shift / 资源时刻）。
...
量法：`../lab/工具/scrollcost.py`（headless，逐个 CSS 变体注入，出 `RasterTask` / `DirectRenderer::DrawFrame` 总量）与 `frameab.py`（真窗口 2560×1600 @165 Hz，rAF 帧间隔 → 掉帧数）。**旧的 `jank.py` 只量主线程计数器，看不见合成器侧**，所以前面「backdrop-filter 测不出代价」的判断按下面这组数字修正——代价是真的，只是没到掉帧。
...
- **宽屏 3 列（下限 190px → 每张 ~205px），窄屏 2 列（下限 130px）**。为什么不要更密的 4 列：两版摆在一起比过（`../lab/结果/cmp-3col.png` 与 `cmp-4col.png`），155px 下十几种工艺糊成一片 —— 和纸的纤维、金继的裂缝、金边的拉丝、珐琅的格线全看不见，而「一眼看出这八张不是同一种卡」正是这一页存在的理由；205px 也更接近首页那张 272px（工艺就是按那个尺寸调的）。代价：63 ÷ 3 = 21 行，页面只有往下滚（桌面整页高 ~8200px）。
...
- **导航图标**：新画了第 9 个像素角色「初音未来」（青绿双马尾 + 新建的 `ribbon` 缎带蝴蝶结配饰，`tools/icons/make-icons.py`）。挑她而不是张数最多的艾米莉亚（27 张，已经站在「关于」那一项上）的理由是**色相**：16px 下认人靠发色，现有八人的发色里没有青绿。预览图 `../lab/结果/nav-collection-preview.png`。
...
- `../lab/工具/scrollcost.py`（headless，合成器侧）：`wall-full` vs 把玻璃的 `backdrop-filter`、厚度层 `.home-card-lens`、金继/星芒的 SVG 位移滤镜**全关掉**（`wall-lite`）vs 把 63 张的覆盖层**全部关掉**（`wall-nolayer`）：`Graphics.Pipeline` 285.9 / 242.2 / 241.4 ms，`DirectRenderer::DrawFrame` 106.0 / 85.1 / 84.0 ms。**lite 与 nolayer 几乎一样** —— 也就是说那三样「听起来最贵」的东西合计只值 ~1ms，真正的开销在每张卡本身的基础混色层。因此**降级玻璃 / 金继不会换来任何东西**，不动它们。
- `../lab/工具/frameab.py`（真窗口 2560×1600 @165 Hz，帧预算 6.06 ms）：`wall-full` p50 6.1 / p90 6.1 / p99 12.15 ms，374 帧里 6.5 帧 >7ms；**第一轮 10 帧 >12ms、第二轮只剩 2 帧** —— 掉的那几帧跟着「首次滚到某张卡」走（图要解码 + 覆盖层要栅格化），不是稳态代价。`wall-nolayer` 387 帧 0 掉帧，即：**整副牌的观感在慢滚动下值约 1.7% 的帧**。
- `../lab/工具/cpu4x.py`（4× CPU 限速，弱设备近似）：p50 6.0 / p90 6.5 / p99 6.9 ms、300 帧里 1 帧 >7ms —— 主线程不是瓶颈（合成/栅格在 GPU 侧）。
- **移动端帧率仍未真机量过**（与 ㊳ 同一条边界）。
...
**实测（DOM 断言，`../lab/结果/filt-after/probe.js`，每步都与「按清单独立算一遍」对照）**
...
- **`:hover` 没法用 JS 合成**（`MouseEvent` 不产生悬停态），扫光必须用 CDP 的
  `Input.dispatchMouseEvent` 真移鼠标 —— 新增 `../lab/结果/deck8/hover.py`（复用 `shots.py` 的
  Edge 启动与 target 选择）。**负对照也别拿「有没有动画」判**：动画在所有档上都跑，可见性由
  `--rank-orn` 决定，收藏档 `opacity: 0` —— 判据要读那一层的 opacity。

**实测（`../lab/结果/deck8/probe.js` + `deeplink.js` + `home.js` + `hover.py`，每步与清单独立对照）**
...
实测（`lab/工具/curve-sheet.py`，DPR2）13px 下双纽线糊成两个点、玫瑰线糊成星号、蝴蝶糊成
...
- **CSS**：`corner-shape: superellipse(4)`（CSS Borders 4）。探针 `CSS.supports(...)` 在
  ...
  角对照图 `lab/结果/deck9/corner.png`（DPR4，超椭圆 vs 强制 `round`）能看出差别：
  超椭圆更早离开直角、把角填得更满。
...
验收 `lab/结果/deck9/card3d.py`：同一档、同一角度，用 `card3d.setFx({coneC:0, holoC:0})`
...
`lab/结果/deck9/depth.py`（只读）按档聚合出四个数：幅度 max−min、倒角带（离边 3~14px 的均值）、
...
**十、验收产物**（都在 `lab/结果/deck9/`，只有 `curve-sheet.png` 在上一级 `lab/结果/` —— 与它的脚本同级）
