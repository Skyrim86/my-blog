...
- 光晕/全息不再「莫名其妙地闪」（2026-09-25）：跟手高光改成**真拖过才亮 + 斜坡**（原来一按下就整张亮，
  量到 9619 像素一帧内变）；星屑改成格间双线性 + 门放宽 + 周期 6.3 s（单像素最大跳 147.7 → 1.9）；
  时间通道加包络，余韵不再「冻在半相位」（`stats().fx.env` 可读，收尾停在 0 才停循环）。
  探针 `lab/工具/flicker2.py`、`lab/工具/envline.py`。
