...
**首页的卡片组（20 张，一次一张、可手动切也会自己轮播）— `_partials/home-cards.html` + `assets/js/home-deck.js` + `21-card-deck.css` + `tools/cards/make-cards.py` + `data/home-cards.yaml`**（2026-09-19 新增，当天改过六轮）

- **形态**：hero 最下面（头像 / 标题 / 副标题 / 「站点规模」那行之下）一张 5:7 的卡，显示约 240×336；底部名牌写**这张卡自己的名字**（`木刀` / `星夜` / `樱堤` …）+ 序号 `07 / 20`，名字下面是系列（绫华 / 黑长直少女）。
- **交互**（都在 `assets/js/home-deck.js`，脚本只在首页加载）：点卡面 / 卡下 `‹ ›` / 键盘 ←→ / 触屏左右滑都能切；每 6s 自动换一张，**悬停、焦点进入、标签页后台都暂停**，手动切过重新计时；`prefers-reduced-motion` 时完全不自动轮播、也不做淡出淡入。
  - **只有当前这一张在 DOM 里**：28 张全渲染的话它们叠在同一位置、全在视口内，lazy 也拦不住；现在是「先下第 1 张 + 预取后 2 张」。
  - 禁 JS 也能看首张卡与序号（切换按钮由脚本注入）；换卡会写进 `.sr-only` 的 `role="status"` 播报「第 N 张，共 M 张：名字」。
- **两张卡一个名字（每张自己一个）**：名字按画面取，往她的意象上靠（白鹭 / 霜 / 扇 / 樱 / 舞）。现在 20 张是：绫华 16 张（木刀 · 墨羽 · 花信 · 书案 · 霜华 · 白鹭 · 星夜 · 玄袖 · 樱堤 · 甲刃 · 蝶羽 · 闲坐 · 午后 · 雪舞 · 流水 · 夏荫）+ 黑长直少女 4 张（日间 · 夜间 · 大小姐 · 提灯）。
  - **像素小人那 8 张已撤**（2026-09-19 按「不要像素小人头」）。样式与脚本里的 `pixel` 模式都留着：想加回像素卡只需在 YAML 里写 `style: pixel`。
  - 黑长直少女这一系刻意不用网上的图（`black_hair long_hair` 的候选池质量很差，见 `../lab/shots/revamp/deck/girl-sheet.png`），用的是**站上自己的角色资产**。
- **取景口径是「人像完整」**（2026-09-19 改，之前是「填满卡面」，结果经常只剩半张脸或切掉腿）：默认 `crop: figure` —— 先按内容包围盒（阈值 45 + 1%~99% 分位）量出人物，再取一个**刚好装下它的 5:7 窗**，用 `pad` 给呼吸；窗口装不下画布时改成「整幅缩进卡面 + 四周垫底色」，底色优先用 YAML 里的 `flat`、否则按**实测四角颜色**取。
  ...
  - 横构图的手绘图（`ayaka-sword`）走手量分数窗：自动取景会把横过来的刀也算进内容，人被缩到很小。
- **十种卡面风格，全是 CSS**（换风格只改 YAML 的 `style`）：`foil` 全息（彩虹膜 + 扫光）· `gold` 金边 · `washi` 和纸（纸纹 + 花瓣）· `frost` 霜蓝（霜晶放射纹）· `ink` 墨 · **`glass` 玻璃**（::before 用 `backdrop-filter: blur(2px)` 把身下的画面**真的糊开** + 一圈内亮边；4px 时人像糊到认不出，实测后收到 2px）· **`gothic` 哥特教堂玻璃**（三向铅条分格 + 金/蓝/玫红三块宝石色 + 暗角）· **`sakura` 樱纹** · **`asanoha` 麻叶纹**（60/-60/0 三向细线的三角格）· **`seigaiha` 青海波**（三块错位半圆径向渐变平铺）。
...
- **代价**：`assets/images/cards/` 里 20 张源资产合计约 600 KB（构建输入、不进访客流量）；首页只加载第 1 张 + 预取 2 张，每张 1x 约 10~15 KB。
- **另一条备选形式（当前关着）**：hero 里一块透明底场景插画（`_partials/home-chara.html` + `hugo.toml` 的 `[params.home].chara`）。
