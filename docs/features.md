...
- **交互**（都在 `assets/js/home-deck.js`；脚本在首页与收藏库加载 —— 后者页内没有轮播卡，只用它的弹层与 3D 查看器，见 ㊿）：
  ...
  - **换卡要等多久（2026-09-22 晚重做，`../lab/结果/deck-perf/check-cardflow.py`）**：这一条从头到尾只讲一件事 —— **别让「按下去了」与「画面换人」之间隔着一次下载**。三处改动：
    ...
    - **低清先上（这条才是主因）**：`card-3d.js` 原来只有「xl 那一张贴图到了才换画面」，所以等多久取决于 760 那一档下多久。现在先上传**页内那张真正下过的档**（首页 `l`/`s`、收藏库格子是 412 —— 由 `home-deck.js` 的 `lowTierOf()` 从 `currentSrc` 读出来，不猜档），它必然在缓存里，几乎立刻换上；xl 到了再换一次，观感是「先变过来、再变锐」。没 WebGL 的机器走平面大图那条路，同样先摆低清再升级。`faceSeq` 防晚到的图盖住后来居上的那一张。
