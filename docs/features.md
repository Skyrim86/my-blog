...
- **角色立绘**（2026-09-19 新增）：hero 里副标题之后一位透明底 Q 版立绘，整块在 `home-chara.html`、图与开关在 `hugo.toml` 的 `[params.home].chara`。纯装饰（`alt=""` + `aria-hidden`）、不包链接、不改任何文字底 —— 加了它之后对比度表逐格未变。细节见 ㉕。
...
**首页的绫华（hero 里的透明底立绘）— `_partials/home-chara.html` + `09-home.css` + `tools/backgrounds/make-ayaka-chibi.py`**（2026-09-19 新增）

- **资产与出处**：源图 `tools/icons/source-ayaka-chibi.png`（Q 版神里绫华，白底 1000×1000，画师 maidsan，pixiv `artworks/92959293`，非商用同人 —— 与壁纸、看板娘同一档许可）。生成的 `assets/images/ayaka-chibi.webp` 是**生成产物，不要手改**。**画师的签名留在图上没裁掉**：同人署名不该抹，出处记在本节。她在管理页那条线里早就是主角（`tools/admin/ui/ayaka-bg.webp`、`ayaka.ico`、冰蓝/霜白/樱花粉那套配色），**前台以前一次都没出现过**，这次是第一次。
- **抠法与看板娘刻意不同**，因为两张画的画法不同（下面每条都有实测数字，换素材前重新量）：
  1. **必须「从画布边缘灌水」，不能用全局色阈值**。看板娘那条判据（背景纯白 255、水手服领子 ≤248，于是按色差切、不要求连通）之所以成立，是因为她身上没有纯白；绫华有：**她的袜子实测 `(255,255,255)`，差 0**，而袜子被鞋与轮廓围着、与画布边缘不连通 —— 全局阈值会把它整个打穿。灌水正好放过它。
  2. **容差 20 是量出来的**：白底与它周围的过渡带 差 0~32，**最淡的头发高光 差 33**（`(222,233,253)`），脸 41、雪花 89、袖子 140。取 20 = 吃掉白底与大部分过渡带、停在 33 之前。**容差不能超过 33**，那会开始啃她的头发高光。
  3. **不能按「暗 = 半透明」反解 alpha**。看板娘那条 `a = (255-灰度)/(255-墨色)` 在这里是错的：那位是墨线压在白上、暗 = 覆盖度高；绫华是**厚涂的淡色**，淡就是她的本色，套那条公式会把整片淡紫头发算成 11% 不透明。这里的做法是：灌水得到前景 mask 后**向内收 1px**（收掉的正是最白的那圈混合像素），再把 alpha 用 1px 高斯软一次。1000px 源图少这 1px，缩到出图尺寸只剩半个像素，肉眼看不出来，白边却没了。
  4. **缩放仍走预乘 alpha**（同看板娘第 3 条），否则透明的白会被平均进轮廓。
  - 预览写到仓库外的 `../lab/shots/ayaka_check.png`（深底 / 浅底各一张），**改完必须看**：这张图的成败只能看图定（头发有没有被啃、袜子还在不在、雪花边缘干不干净）。顺手量过三种缩放核（LANCZOS / BICUBIC / BOX），不透明像素的最低亮度 0/1/2、暗像素占比 6.7/6.4/6.3% —— 放大图上看到的「暗边」是原画自己的轮廓，不是缩放振铃，**不用为它换核**。
- **位置与尺寸**：hero 里、副标题之后、站点规模之前（流内布局，不做绝对定位 —— 那会压到右栏面板上）。宽度分档：窄屏 128 / 641px 起 168 / ≥1024px 用 `width: auto` 回到模板给的 `charaWidth`（CSS 不写第二份）。出图 = 显示宽度的 **2 倍**（脚本的 `OUT_W`，改显示宽度要同步改它）。开关在 `hugo.toml` 的 `[params.home]`：`chara`（**留空则不显示**）、`charaWidth`。图必须放 `assets/` 下（放 `static/` 会让 Resize 静默忽略，traps 第 22 条）。
- **主题给头像写的两条规则会命中「hero 里任何一张图」**：`.profile img { border-radius: 50% }`（`profile-mode.css`）与 `@media (max-width: 768px) { .profile img { transform: scale(.85) } }`（`zmedia.css`）。覆盖必须写 **`.profile .home-chara`**（(0,2,0)）—— 只写 `.home-chara`（(0,1,0)）压不住 `.profile img`（(0,1,1)），表现是「她被裁成圆形」或「窄屏尺寸跳一下」，都不报错。
- **分离用投影按主题分**：浅色主题给**两道**（`0 0 1px` 的紧边 + `0 6px 16px` 的松影）—— 实测只留松的那道时，她头顶与白天套的亮天空交界处仍然发糊；深色主题给冷色微光 + 深影（与看板娘同一套手法）。
- **代价**：只加载在首页（其它页不请求）。产物里 **1x 15.8 KB / 2x 45.1 KB**（Hugo 按显示宽度重编过，比源资产 43 KB 还小），普通屏访客付 15.8 KB；对比：看板娘 41 KB 是每页都下。
- **同屏人数：2026-09-19 明确选择「都留着不避」** —— 壁纸少女 + 右下角看板娘 + 首页绫华，一屏三位。因此约定：三人分居不同区域（壁纸人物在右侧中景、看板娘固定右下、绫华在 hero 左栏）、尺寸差一档，**不要再往画面中间加人**。想收就改 `defaultPreset` 回 `'city'`（壁纸里没人）或把 `params.home.chara` 留空。
