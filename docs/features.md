...
reduced-motion → 整支脚本不挂载；无 WebGL / GLSL 编译失败 → 同样不挂载。这三种情况下由脚本里的
`standDown()` 把 `data-bg` 从 `__shader` **换回静态兜底套**，壁纸自然回来。复跑：
`lab/结果/bg-shader-contrast/check-degrade.py <url>`（三种情况各起一个 Edge，判据是 `__bg.why` 与 `--bg-image-light` 还是不是 `url(...)`）。
...
**对比度也是量过的**（同一套口径，逐帧取底色众数、排除笔画像素）：**深色主题最差 6.08、浅色 5.72，都高于静态壁纸的 5.31 / 4.82**
（AA 线 4.5）。浅色主题是风险面（深字 + 会变暗的底）：只有一套深色配色时浅色主题有 28 项掉到 4.30~4.49，所以落地版按
`data-theme` 切两套配色。复跑：`capture-contrast.py` + `analyze-contrast.py`。

**浅色那套的两个做法**（2026-09-22 晚：第一版「看不出来」之后的返工）：
① 光带色做**亮度中性提饱和** `c' = luma + (c-luma)*k`（k=2.3）—— WCAG 只看亮度，这个变换在数学上保持亮度不变，
所以彩度可以放开提而对比度一分不掉（6.12 → 5.72 那 0.40 全部来自「底色压暗一档」，与提饱和无关）；
② 上色方式从「把底压暗一层」改成「按 glow 在底与光带色之间插值」—— 压暗得到的是灰雾，插值得到的才是带颜色的光带。
**注意文字并不直接坐在背景上**（正文都在 0.8 不透明度的玻璃面板里），所以背景的亮度变化对实测对比度影响被面板挡掉了大半 ——
真正决定那 5.72 的是面板自己。这也意味着「浅色光带能画多明显」的约束不在文字，而在观感协调。

**动态背景也是可切换的一「套」（2026-09-22 晚补）**：`extend_head.html` 往切换脚本的 `data-presets` 里追加一项
`{id: '__shader', name: <shader.name>}`，首屏默认值取 `shader.default` —— 于是那个按钮的循环变成
「动态 → 黑长直少女 → 城市 → 动态 …」，访客自己切过之后记在 `localStorage['pref-bg']` 里（与原来的套同一份偏好）。
`bg-switch.js` 负责切到 / 切离它时调 `__bg.start()` / `stop()`（`stop()` 后 rAF 停，GPU 一秒都不花），
预取会自动跳过这个没有 URL 的项去找下一张真图。判据脚本：`lab/结果/bg-shader-contrast/check-switch.py`。
彻底关掉动态背景 = `enable = false` 重新部署。
