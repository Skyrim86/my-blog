...
- **交互**（都在 `assets/js/home-deck.js`；脚本在首页与收藏库加载 —— 后者页内没有轮播卡，只用它的弹层与 3D 查看器，见 ㊿）：
  ...
  - **换卡是交叉淡入 + 方向位移**：`.home-card-ghost` 装住**刚显示过的那一张**（URL 已在缓存里，不多下一次），主图立刻换成新的，两张在 320ms 里交叉 —— 中间没有空白帧。上一版是「先淡出到 10% → 200ms 后换 src → 再淡入」，那 200ms 里卡上几乎没有东西（traps 第 86 条）。`.is-next` / `.is-prev` 给位移定方向：新卡从来的那一侧滑进来、旧卡往反方向退，「上一张 / 下一张」在视觉上分得出来。
  - **只有当前这一张在 DOM 里**：63 张全渲染的话它们叠在同一位置、全在视口内，`loading=lazy` 也拦不住；现在是「先下第 1 张 + 预取后 2 张」（残影只在换卡那 320ms 里显形，装的是已缓存的那张）。
...
**浮雕是两份数据叠加**：`tools/cards/make-depth.py` 用 Depth Anything V2 Small（走 **hf-mirror.com**，`huggingface.co` 在本机不通）为每张卡面离线生成一张**灰阶高度图**（600×840、与卡面同尺寸，着色器用同一套 UV 采样，不需要任何对齐逻辑）。产物的路径**由 `image` 字段推导**（`images/cards/depth/<同名>.webp`），不是新加的 YAML 字段 —— 手写路径字段会漏、会与产物改名脱节。体积账：灰阶 q75 实测**约 8 KB/张**（卡面画作本身约 43 KB），63 张合计 **520 KB**。
...
**三个观感参数都是量出来的**：主体凸起 2.2% → **3.2%**（5% 那版整张卡像立起来的纸板剪影）；布光系数之和校准到「正面朝相机、无起伏」时约 1.0（第一版加起来 1.28，整卡过曝）；高光指数 72 → **150** 且只在抬起的部位增强（72 太宽，卡面是一大片平面，`N·H` 整片都高 → 泛白）。
...
**空闲时完全停掉动画循环**：只在拖拽 / 惯性 / 回弹 / 指针悬停时 `requestAnimationFrame`，一旦静止就停 —— 这是博客首页的一个弹层，不该在没人动的时候一直烧 GPU。`card3d.stats().running` 与帧数增量就是给这条断言用的。
...
**实测**（1440×950、`--force-device-scale-factor=1`，证据在 `../lab/shots/deck3d/`）：拖动 14 帧的覆盖率最低 9.3%（**没有空帧**，判据是 α>128 的像素占比而不是「截图看着不黑」）；松手精确停在 -π（背面），翻面按钮两个方向都对；静止后 900ms 内**帧数增量为 0**；关弹层后同样为 0；控制台 0 错误 0 警告。角度对照 `angles-light.png`（0/26/46/63/90/132/166/180°，90° 时覆盖率 3.8% 就是那条真实的卡边）、明暗两套 `open-sw-light.png` / `open-gpu-dark.png`（后者在真显卡上拍，与软渲染的读数几乎一致）。

**体积**：整站 gzip **4742 / 5120 KB**（这一轮 +179 KB：深度图 150 KB + 脚本与样式约 15 KB，加上 P5 那一批新卡前的余量 378 KB）。

**已知边界**：`POM`（片元侧的细部光线步进）在 `hardwareConcurrency ≤ 4` 或视口小的一侧会关掉并把网格降到 28×40；移动端的帧率没有真机量过（本轮只量了桌面）。风格参数表 `STYLE_3D` 与 YAML 的 `style` 字段必须一一对应，少写一种会静默套用兜底 —— `check-deck.mjs` 里有守卫核这两边的键。
...
**显形有两条入口**：在弹层里**累计转满 360°**（`turned` 累加拖拽角度），或**在背面停留 1.5 秒**。两条都验过，并且换到别的卡再换回来会重新藏起来。停留那条**必须用 `setTimeout`**：卡停稳后渲染循环就停了，写在 `update()` 里的判据永远不会执行（traps 第 94 条）。

#### 工艺的两档：普通 / 进阶，与「进阶 = 雕花框」（2026-09-21）

用户给的下一步是「工艺分普通与进阶两档（星芒全息适合进阶）」，并且要一副**雕花框**（附了一张参考图：
暗底卡片 + 多层鎏金雕花边 + 角上镶宝石 + 一圈细亮内线）。这两件事做成了一件事：

- **分档口径取「能从代码里数出来」的那个**：普通工艺只有一层（纸底 + 单层印纹或单层闪），
  进阶工艺要多层叠加。**普通（5）**：`foil` 全息 / `washi` 和纸 / `ink` 墨 / `yukika` 雪华 /
  `starnight` 星屑；**进阶（6）**：`holo-prism` 星芒全息 / `glass` 玻璃 / `gold` 金边 / `kintsugi` 金继 /
  `filigree` 雕花金 / `enamel` 珐琅彩。（玻璃是全文件里最重的一种 —— 四件套 + 透镜层 —— 所以它在进阶。）
- **两档不只用来分类**：进阶工艺的卡长雕花框。两个令牌：`--craft-ornate`（0/1，总开关）与
  `--craft-band`（边栏宽度，默认 20px）。`--cart-inset` 里那个 `max(--rank-mat, 边栏宽)` 是关键 ——
  **普通档仍是等级给的画面内缩**（六档 14→5px 那条阶梯照旧），进阶档的内缩改由边栏宽度决定
  （约 20px + 框棱），因为雕花边栏需要够宽的带子（参考图里约占卡宽 8%）。代价是进阶档上
  「画面逐级出框」让位给了雕花框；等级在那些卡上仍由颜色、框棱、工艺浓度、覆膜、徽章五个通道表达。
- **雕花框拆成四件**：细亮内线（`.home-card-mat`，本来就有）＋ 暗底边栏（`.home-card-fret::before`，
  四条纯渐变 mask 拼成的「回」字）＋ 金色雕花线与角花（`.home-card-fret::after`，八层 mask：
  上下边栏平铺、左右边栏平铺、四角角花）＋ 四角宝石（`.home-card-rank::after`，颜色由
  `--craft-gem-color` 给）。**刻意不用 `mask-composite`**：那条属性在旧 Chromium 上要写
  `-webkit-mask-composite` 的老关键字，两者语义不一致，写错就是整条边栏静默消失；四条矩形带的
  并集用四层 mask 就够，不需要布尔运算。
- **必须显式给 `z-index`**（实测踩到）：卡面上有几个风格层挂在 `::before` / `::after` 上
  （玻璃的镜面高光、金继的暗侧、全息的扫光），而伪元素的绘制顺序排在**所有子元素之后** ——
  不显式分层的话它们会盖在雕花框上（玻璃那条高光是 `screen` 混合，把深色边栏整片提亮，
  雕花框**直接看不见**）。现在三层：框底+线 `z-index: 2` → 等级覆膜与宝石 `3` → 名牌 `4`。
- 宝石的颜色不能由等级给（等级那套是金属色阶），所以单独一个令牌；雕花金用琥珀、珐琅彩用蓝紫。
- **每种进阶工艺各有一副自己的框**（用户第三次追加：「边框应该和卡片风格匹配」）。做法是把框的
  **零件**拆成可按风格覆写的四个令牌，结构那八层 mask 不动：`--fret-rail-h` / `--fret-rail-v`（边栏瓦片）、
  `--fret-corner`（角花）、`--fret-line`（线的材料，接受颜色**或渐变**）、`--fret-band`（边栏底）。
  于是四种语气各一副框：

  | 工艺 | 边栏 | 角花 | 线的材料 | 边栏底 |
  |---|---|---|---|---|
  | 雕花金 | 卷草（参考图那副） | 圆花心 | 金 `#e9ca86` | 近黑 |
  | 金边 | 规整线条款（三线 + 双排珠） | 圆花心 | 亮金 `#f0d9a0` | 暖黑 |
  | 玻璃 | 规整线条款 | 圆花心 | 冷白半透明 | **半透明冷灰**（透出画面） |
  | 金继 | **裂缝款**（棱角裂线 + 细枝） | 圆花心 | 旧金 `#e5b96a` | 近黑 |
  | 星芒全息 | **斜向棱纹**（纯 CSS 渐变当 mask，0 新增图） | **棱角碎星** | **彩虹渐变** | 珠光浅 |
  | 珐琅彩 | 规整线条款（珠子读作掐丝的格） | 圆花心 | 亮金 | 深蓝紫 |

  新增的生成物零件：`--tex-fret-rule`（线条款，横竖各一）、`--tex-fret-crack`（裂缝款，横竖各一）、
  `--tex-fret-shard`（棱角碎星角花），约 2 KB。星芒全息那条棱纹**不用图**（CSS
  `repeating-linear-gradient` 本身就是合法的 mask 图）—— 这是 `mask` 比 `border-image` 好用的地方：
  任何 `<image>` 都能当遮罩。守卫补一条：**凡是 `--craft-ornate: 1` 的风格必须显式声明
  `--fret-line` / `--fret-rail-h` / `--fret-corner`**（漏写＝沿用默认那副卷草框，页面上只是「金得不太对」，
  看不出是漏了）。反向测试过（抽掉金继那三个令牌，两条都被拦下）。
...
**取景改成填满**：原口径「整幅装进卡面、装不下就补边」实测让 **32 张里 20 张**露出空带。现在的优先级是用户定的：**先保头像完整、其次半身完整** —— 窗口按人物宽度定、纵向裁成胸像，头不能被切（结构性保证，见 traps 第 92 条）。「头顶留白」是生成器逐张打印的可核对数字，低于 1.5% 会打 ⚠（实测有两张是源图本身顶部就裁得紧，不是我裁的）。
...
**逐像素核对**（40 张：4 页型 × 5 宽度 × 明暗，关掉滚动位置恢复后拍，做法见 traps 第 98 条）：列表页、课程材料页的 380/640/1024 全部 **0.00%**；首页的 0.26~0.37% 是头像光环（见 ㊸）；**只有课程材料页在 1240/1440 有 1.5% 的差异，位置全在左侧目录栏内部**。查过了：它的几何量改前改后逐字相同（`rail` = [84, 136, 200, 842]、`scrollTop` = 0、首个链接 [123.88, 150.59]、`documentHeight` = 16115），所以那不是位移，是**吸顶改变了合成层、目录栏文字的抗锯齿跟着变了一档**。
...
- **减少动效偏好下整个脚本不介入**（连 IO 都不建），`22-reveal.css` 那段也包在 `prefers-reduced-motion: no-preference` 里，两层都挡。**实测**（用 `shots.py` 新加的 `--reduced-motion`，见 traps 第 99 条）：普通模式下课程章节页载入时 `.reveal-pending` = 2，模拟 `reduce` 后 = 0。
...
**顺手修掉的一个真坑**：加这层壳时，`.home-hero > span` 这个**结构选择器**把它一起命中了 —— 那条本来是给副标题写玻璃底衬的（`00-theme.css`），另外 `09-home.css` 里还有三条（副标题样式、入场动画两条），四处同时误伤，表现是头像凭空多出 `.12em` 的内边距和一块玻璃底，**四份规则一个都没报错**。修法是给副标题加了类名 `.home-hero-subtitle`，四处一并改成类名。教训见 traps 第 97 条。
...
**逐像素核对**（40 张：4 页型 × 5 宽度 × 明暗；关掉滚动位置恢复**并派发一次 scroll 事件**后拍，做法见 traps 第 98 条）：列表页与课程材料页在 380/640/1024 **全部 0.00%**。有差异的四处全部查过、全部可解释：首页 0.26~1.9%（头像光环，见 ㊸）；课程材料页与关于页在 1240/1440 的 0.6~1.8%（左侧目录栏，几何量逐字相同，见 ㊶）；关于页在 640~1024 的 0.15~0.24%（评论区容器把那段往下推了 26px，是本次的意图）；关于页 380 的 0.36% 是 `## 本站有什么` 那个 h2 **被多画了 1 像素**。
...
- 两个最紧的元素**都在面板里**（时钟卡），而面板内的真实对比度比脚本的模型**更高**
  （`backdrop-filter` 的模糊会把底下的亮点抹匀，见 traps 第 73 条），所以真实值还有余量。
...
- 体积：整站 gzip 从 6424 涨到 **6427 KB**（预算 6656，余量 229 KB）—— 增量只有 3 KB，
  ...
  样式并进了 `09-home.css`），所以没有 traps 第 115 条那种「每个标签约 180 字节/页」的固定开销；
...
- **踩到的坑：压缩器会把 `@view-transition` 整条丢掉**。它原本放在 `css/extended/` 里（和其他
  ...
  名字和动画都在、转场永远不触发、**不报任何错**（见 traps 第 101 条）。所以这个文件单独放 `assets/css/`、
...
- **筛选条由脚本注入、项是现算的**（`deck-wall.js` 从清单里数 series / styleLabel / rankLabel）：加一张新系列的卡，筛选条自己多一项，不会出现「写死清单里挂了零张的项」。三排之间 **AND**、每排内部单选（再按「全部」等于取消这一维），结果张数写进一个 `role="status"` 节点播报，无结果时给一句话而不是空白。
- **工艺的中文名第一次落地**（i18n 的 `deckStyle*` 八条）：此前 `style` 只有代码名（foil / washi / kintsugi…），中文叫法只存在于 CSS 注释里。用**「工艺」**而不是「风格」：全息膜、和纸、金继本来就是做卡的工艺，读者不必先学一套内部词汇。
...
| 筛选 | 「玻璃」→ 共 7 张（与 YAML 分布一致）；「初音未来 + 玻璃」→ 共 0 张 + 空态；三排都回「全部」→ 共 63 张、0 隐藏 |
