...
|---|---|---|
| 传世（relief 0.081、18 步、五条通道全开） | 6.1 / 6.1 / 6.2 / 6.2 ms | **0** |
| 奇迹显形后（relief 0.093、20 步、通道全 1） | 6.1 / 6.1 / 6.2 / 6.2 ms | **0** |
| 同上、**4× CPU 限速** | 6.1 / 6.1 / 6.2 / 6.2 ms | **0** |
...
`steps` 的顶端（24）在 2026-09-21 被**降回 20**：24 步在 165 Hz 的帧预算（6.06 ms）下越线，
实测 p50 8.3 ms / 211 帧里 179 帧超预算；收到 20 就回到 p50 6.1 ms、0 帧超预算（见 ㊳ 的性能表）。
