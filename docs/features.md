...
- **十四种卡面风格，全是 CSS**（换风格只改 YAML 的 `style` 字段，不重出图）：`foil` 全息（软边虹彩带 + 光栅颗粒 + 悬停时带位平移）· `gold` 金边（拉丝金属框 + 斜向柔光 + 描金角）· `washi` 和纸（**各向异性**纸纤维 + 纸浆云斑）· `frost` 霜蓝（收尖的软扇骨：6 主枝 + 12 短枝）· `ink` 墨 · `glass` 玻璃（**四件套**：倒角 + 柔和高光 + 底边折射亮线 + 假厚度，磨砂只做下半张）· `gothic` 哥特教堂玻璃（手抖过的铅条 + 四块宝石片 + 暗角）· `sakura` 樱纹（五片**真花瓣轮廓**）· `asanoha` 麻叶纹（软笔触的三向格 + 手抖）· `seigaiha` 青海波（渐隐的半圆环）· `yukika` 雪华（**带分枝的六角雪花**平铺成雪野）· `nishiki` 织锦（绸面柔光 + 纤维 + 斜纹暗示）· `kintsugi` 金继（黑底 + 4~5 道**弯曲分叉**的金缝）· `uchiwa` 团扇（14 根**弓形**扇骨 + 扇箍 + 纸面透光）。另有 `pixel` 备着（当前没有卡用它；生成器的 `pixel` 通道与样式都留着，它的硬边是题材本身、不挂手抖滤镜）。
