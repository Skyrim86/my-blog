...
- **空闲预取另一套**：加载完成、浏览器空闲且**页面在前台**时，把「另一套 + 当前主题」那一张用 `new Image()` 取回来并 `decode()` —— 点切换时图已在解码缓存里，是秒切。**只沿当前这条轴预取**（2026-09-22 晚第四版）：动态背景开着时空闲期**一张静态壁纸都不下**（那是一时刻里两根轴同时占着下载与解码位图），改由**意图预热**接手 —— 悬停 / 聚焦 / 按下任一个背景按钮时，才去取「这一下点下去要落到的那张」（停在最后一套时点动态按钮 = 关，落到的就是当前那张壁纸；`warmStaticIntent` / `warmDynIntent`）。见本节「两个按钮」③ 与 `check-prefetch.py` 的 4 跑对照。跳过三种情况：`navigator.connection.saveData`、`effectiveType` 是 2g/3g（桌面浏览器基本不报这个值，尽力而为）、以及页面在后台（改为 `visibilitychange` 回到前台时再补）。明暗按钮改的是 `data-theme`，所以用 `MutationObserver` 盯着它——换了主题，「另一套那一张」就换成另一张图了。
  - 图片来源是**模板给**的：`extend_head.html` 把每套两个主题的指纹 URL 一并写进现有的 `data-presets` JSON（每页多约 110 字节/套）。脚本不自己拼 URL —— 指纹在构建期算，JS 拼不出来。
  - **代价要认清**：看着壁纸的访客会多下一张（**日间 63 KB / 夜间 128 KB**，对照：默认套自己那张是 93 KB / 137 KB）；第四版起动态开着时**不再有这笔**（见 ③）。只在空闲且可见时发生，所以不推迟 `load`；下面那条 4G 首页传输「约 232 KB → 295 KB」的读数取的是**首屏给静态壁纸**的那一版，`dynamicDefault` 上线后首屏根本不下壁纸，那笔预算的对照数改看本节末尾的 744,974 → 582,605 B。想收可以改两处：只对非触屏预取、或把 `onIdle` 的延迟拉长。
...
**两条轴互相独立（各存各的偏好），但同一时刻只显示一套**（2026-09-22 晚**第五版**的口径，用户原话：
「静态壁纸和动态壁纸是独立的，点了静态动态就消失，反之亦然」）：

- 点 `#bg-switch`（壁纸）**会把动态背景关掉**。动态开着时这一下**不换套** —— 按钮上写着哪张就显示哪张
  （一次点击只做一件事；再点一次才轮到下一张）。动态本来就关着时照旧循环下一套。
- 点 `#bg-dyn` 打开动态时，壁纸从画面上消失 —— 这不靠 JS 改行内变量，靠的是生成 CSS 里那条
  前缀选择器（`:root[data-bg^="__dyn-"]{--bg-image-*:none}`），于是那一张图这次**连下都不下**。
- `pref-bg` 因此有两个含义：屏幕上放着的那张，或（动态开着时）**关掉动态之后要回来的那张**。
  关动态回到的永远是你自己选的那张，不是默认套（`bg-shader.js` 的 `standDown()` 同样优先换回它）。

**这三版的来回值得留着**（同一个按钮上返工三次，每次都是口径变了、不是实现变了）：

| 版本 | 口径 | 为什么被推翻 |
|---|---|---|
| 一版 | 一个按钮的循环里混着动态套 | 「换壁纸」与「开关动态」挤在同一根轴上，想固定看某张、又想偶尔开动态，得点着数圈 |
| 二版 | 拆成两个按钮，但**点壁纸顺带关动态** | 被指为拿一次隐式状态改写换「按钮看起来有反应」 |
| 三版 | 两根轴**零交叉写入** | 动态开着时点壁纸画面完全不变，只靠按钮名 + 播报说「关掉动态才能看到这张」——不是他要的 |
| 五版 | **互斥**：点壁纸就关掉动态 | 当前 |

第五版的播报：点了壁纸而动态刚才开着时，补一句 `bgDynOffNote`（「，已关掉动态背景」）—— 不说的话
读屏用户只会听到「壁纸已切换到 X」，不知道画面为什么整个变了。第二版那套「按钮名立刻更新」的反馈
不再需要（现在一点就换过去了），`bgStaticHiddenNote` 这个 i18n 键已删。

**预取口径没跟着改（第四版，仍然有效）**：动态开着时**空闲期不下壁纸** —— 同一时刻只让一张背景占着
下载与解码位图；跨轴那两张改由「悬停 / 聚焦 / 按下」的意图预热接手。实测（`check-prefetch.py` 四跑，
起点都是动态开着）：

- 空闲 12 s 不交互：**0 张**壁纸落袋 —— 首页总传输 744,974 → 582,605 B（浅色 −162 KB / −21.8%）；
- 悬停 → 点击关掉：快网 **284 ms**、512 kbit/s 下 **270 ms**（都落在交叉淡入那 ~250 ms 的地板上；
  前提是提前量 ≥ 下载时间：深色那两张各 ~137/128 KB，512 kbit/s 下要 ~3.5 s，实测 2.5 s 是不够的）；
- 不悬停直接点（慢网对照）：**875 ms**，顶到 600 ms 上限才切、那张图点击之后才开始下。

触屏没有悬停 —— 按下去那一刻才开始，慢网上就是要等；这一笔换的是「不点就不下」。
- 兜底路径也不覆盖静态侧：`standDown()` 优先换回**访客自己存过的**那张壁纸（`localStorage['pref-bg']`，
  ...
  会被悄悄换回默认套。
- 「动态按钮只有在 shader 真的挂上了才建」这条口径在第四版变了一半：引擎改成**惰性挂载** —— 静态壁纸的
  访客不建 `<canvas>`、不取 GL 上下文（本机实测省一次 `getContext`：首次 28 ms、其后 8~15 ms，另加一块
  显存），加载后要等**访客够到动态按钮**（悬停 / 聚焦 / 按下 → `window.__bg.prepare()`）或 `data-bg` 本来就是
  动态套才挂；切回静态套时 `unmount()` 连上下文一起撤（`remove()` + `WEBGL_lose_context`，移动端的上下文
  额度按个数算）。于是「跑不了」往往到挂载那一刻才知道 —— `window.__bg.ok` 变成**乐观**语义（只有
  reduced-motion 能在加载前判死），失败时 `bg-shader.js` 把 `data-bg` 换回静态套并派发 `bg:dyn-unavailable`，
  切换脚本据此**撤掉那个按钮** + 播报一句（`bgDynUnavailable`）—— 撞上时按钮在点之前就消失，不留
  「点了没反应」的控件。回归测：`check-degrade.py` 的 ④ / ④′ 两档（静态访客加载后无画布、悬停才挂且不激活；
  无 WebGL 时悬停即撤按钮）。
...
`check-switch.py`（**两根轴各管各的**：5 步断言 + 两步「动态开着时点壁纸，画面上无动作」的静默检查）、
`check-degrade.py`（降级链 **9 档**：前 7 档是 reduced-motion / 无 WebGL × 首访 / 存过偏好，后 2 档是第四版的
惰性挂载 —— 静态访客加载后 DOM 里没有画布、悬停才挂且不激活；无 WebGL 时悬停那一刻就把按钮撤掉）、
`check-prefetch.py`（**跨轴不预取 + 意图预热**：快网/慢网 × 有意图/无意图，判据含「空闲不交互 12 s 落袋 0 张」）、
`analyze-contrast.py`（对比度）。
...
- **传输增长全部来自预取那一张背景图**：日间城市 63.0 KB、日间少女 93.0 KB（当前生效的那张）—— 也就是说 295.5 KB 里有 156 KB 是壁纸，其余是主样式 59.5 KB、看板娘 41 KB、头像 4.7 KB、脚本与导航小人各 1 KB 上下。**要不要这笔预取是产品决定**（见 ⑫ 那条），不是性能问题：它发生在 `load` 之后，不推迟任何加载指标。（**这一段的读数取的是首屏给静态壁纸的那一版**：`dynamicDefault` 上线后首屏
  根本不下壁纸，跨轴那两张也已经在 2026-09-22 晚第四版里改成空闲期不下 —— 同一个首页前后两跑：744,974 B（跨轴空闲预取）
  → 582,605 B（第四版）。看板娘与主样式那几笔不变。）
...
> 2026-09-26 更新：M1 三页按 § 拆页并重编号（一）–（五）后，全章最大 **29701 → 18686**（homework → 作业（一）14014 / （二）15890；notes-02 → （三）15262 / （四）13111；notes → （一）8431 / （二）18686；notes-03 改题（五）17911、未拆）。全站重排（205 页）前四变为 问题一 22057 > 问题三_证明_下界 19836 > 问题二 19635 > 问题四 19620，**M1 退出最重四页**；>15000 的仍 11 页（含新拆出的 notes-04、homework-02）。
...
### 收藏库的玩法层：探索进度 / 今日一抽 / 看工艺（2026-09-25）

三条一起上，都是**前端 + 数据**，没有新素材、没有新依赖、整站体积没有可测的变化。

| 玩法 | 用户看到什么 | 实现落点 |
|---|---|---|
| 探索进度 | 格子右上角一个小点（看过）／再多一圈（看过卡背）；结果行下面一条 `已探 12 / 57` | 写：`home-deck.js`（localStorage `deck:seen`，键 = 卡的 id）；读与画：`deck-wall.js`；两者的接口是 `deck:progress` 事件 |
| 今日一抽 | 角落那枚像素小人，原来叫「随机翻一张」 | `deck-wall.js` 按访客本地日期洗一遍当前筛出来的，给下一张没看过的；种子函数与 `home-deck.js` 的 `hash01/daySeed` **逐行同源**，`check-deck.mjs` 有守卫 |
| 看工艺 | 弹层里第二颗按钮：固定斜角 + 1.8× 近观 | `card-3d.js` 的 `setInspect()`（机位距离除 `zoom`，阻尼 8/s）；按钮在 `home-deck.js` |
...
- **弹层信息行不再重复系列**：「角色」与「系列」相同（35 张）或系列是角色名的一部分（18 张，绫华 ⊂ 神里绫华）时
...
- 光晕/全息不再「莫名其妙地闪」（2026-09-25）：①跟手高光改成**真拖过（>3 px）才亮 + τ0.14 s 斜坡**
  （原来一按下就整张亮，量到 9619 像素一帧内变 → 现在 0）；②星屑从「每格白噪声 + 两道硬门」
  改成**格间双线性 + 门放宽 + 周期 6.3 s**（单像素最大跳 147.7 → 1.9，星屑本身没减：局部极亮点 520 颗）；
  ③调速器档位加**生效权重斜坡**（τ0.22 s）：开卡那一下 tier 0→1 原来是一帧整体降 15%，现在 1 s 收敛。
  探针 `lab/工具/flicker2.py`、`lab/工具/govline.py`。
