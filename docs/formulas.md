...
`node scripts/check-math-syntax.mjs` 扫 `content/**/*.md` 的数学区，阻断三类：`{}` 不配对（嵌 `$` 的指纹）、**某一行 `$` 的个数为奇数**、`§` 与圈号；输出**公式本体**的 `文件:行:列` 与区域片段，退出码 1。它**只检查不修改**（这几类语义上没法机械替换）。

`bash scripts/push-blog.sh` 与 CI（`.github/actions/validate/action.yml`）都会跑它，正常入口下不需要手动调用。它还顺带补了 `hugo` 的一个盲点：**Hugo 一旦报渲染错误就会取消剩下的页面，列出的坏页可能不全**（实测 3 个坏页只报出 2 个），所以「修完报出来的错误」不等于构建就能过——修完仍要构建到绿。
