...
| 单页的左侧目录栏不跟着滚（或整栏跑到正文末尾） | `.post-single` 上的 `backdrop-filter` 会成为 fixed 后代的**包含块**——留在正文容器里的 `position: fixed` 是相对卡片定位的，于是跟着正文一起滚 | `layouts/_partials/toc-rail.html` 里**内联**的那段脚本把 `#toc-rail` 挪到 `<body>` 下；显示与否挂在 `body.has-toc-rail` 上，没 JS 时不显示（正文顶部的折叠目录照旧）。见 [`features.md` 第 3 节 ㉓](features.md)。**2026-09-19 补**：玻璃透镜把那几处面板的 `backdrop-filter` 覆盖成了 `none`（见 ⑫ 末段），所以开着透镜时这个包含块并不存在 —— 但那段脚本**照旧保留**（它同时兜住「以后谁又给祖先加回 backdrop-filter / filter」的情况，而且目录栏挂在 `<body>` 下本来就与页面结构更吻合）；别看到「现在没有 backdrop-filter 了」就把脚本删掉 |
...
| 窄屏（≤640px）下面板既没有透镜、也没有原本的毛玻璃，比改造前更透，而构建全绿 | `:is()` 的权重取参数里**最高**的那个：透镜基础规则的长列表含 `article.post-entry`（元素+类）= **(0,2,2)**，而降级段写 `:is(.post-single,.page-header)` 只有 **(0,2,1)** —— 低一档，基础规则的 `backdrop-filter:none` 一直赢。2026-09-19 实测：375px 下 `getComputedStyle('.page-header').backdropFilter` 是 `none` | 基础规则与降级规则**成对使用同一条选择器**、靠「后出现」取胜（`20-lens.css` 里已分组成对写）；改完在 375px 下回读 `backdropFilter` 断言 |
| 以为「滚到某处再截图」做了滚动测试，其实页面根本没滚（不同 scrollY 的截图逐字节相同） | `html { scroll-behavior: smooth }` + 隐藏标签页里 rAF 被节流 → 平滑动画完全不推进，`scrollTop = y` 赋值静默无效。2026-09-19 就因此得到过一个假的「滚动不变性」结论 | 测前把 `scroll-behavior` 覆盖成 `auto`，并且**回读 `scrollTop` 断言真的滚了**再取图。第 6 节「量滚动卡顿的两个坑」是同族的另两个变体（真窗口被遮挡时 rAF 冻结拿到 0 帧；逐帧 `scrollBy` 被平滑动画吃掉） |
| 用 JS 设了 `data-theme` 再截图，拍出来还是旧主题 | 页面脚本会按 `localStorage['pref-theme']` 把主题刷回去 | 设 `data-theme` 的同时写 `localStorage`，并回读 `getAttribute('data-theme')` 断言（三次切换要三次都对） |
| 想在 `backdrop-filter` 里引用 SVG 滤镜做「真折射」，完全没效果、也不报错 | `backdrop-filter` 的语法允许 `url()`，但 Chromium **不处理这个引用**（实测：同一条纹 `url(#lens-panel)` 的像素标准差 111.88 ≈ 没滤，`blur(8px)` 是 5.79） | 只能复制一层背景图、再对它用 `filter: url(#…)`；做法与参数见 [`features.md` ⑫ 末尾](features.md) |
| 照规范写了「`filter` 指向不存在的定义时元素完全不渲染」，实测不对 | Chromium 下那只是**当作没有滤镜、照常渲染**（实测红色方块均色仍是纯红） | 别把规范当实测抄进注释；要兜底就另找理由并写清楚（透镜那边是「面板本体留底色」，属防御性，理由已注明） |
| 听信「`transform` 会打断 `background-attachment: fixed`」，为它删掉 hover 抬升 / 改设计 | 2026-09-19 实测**不成立**：元素自身 identity 变换、自身 `translateY(-2px)`、祖先 `translateY(0)`、祖先 `filter: blur(0)` 四种情形下背景都仍锚定视口（滚动前后同一屏幕区域差异 98~100%，被打断才会接近 0） | 别为这条改设计。真会打断锚定的是 **iOS Safari 把 `background-attachment: fixed` 当 `scroll`** —— 那条由 640px 断点的降级处理 |
