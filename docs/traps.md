...
- **「接入 Lighthouse CI」收益低于成本**：站点已有 12 步校验（含 `report-size.sh` 的单页/整站/索引体积预算）与每周 lychee 外链检查；Lighthouse CI 在 Pages 子路径下需要额外的 server 与 token，且它的产出与本地 `../lab/结果/` 那套手测量法重复
