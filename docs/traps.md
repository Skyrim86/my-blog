...
| 背景套切换**卡在中途**：旧壁纸的临时图层（`.bg-ghost`）留在页面上、`data-bg` 一直不换、也没报错 | 关键流程只挂在 `requestAnimationFrame` 上。**标签页在后台时 rAF 完全不跑**（不进队列，不是「慢」），于是「等一帧再动手」的那一步永远等不到；而定时器在后台会被节流到约 1 秒，所以只加 rAF 的兜底等于没有兜底（2026-09-19 实测：`visibilityState` 是 `hidden` 时切换停在半路，切回前台约 1 秒后才补完） | 「等一帧」一律写成 **rAF 优先 + `setTimeout` 兜底**（`bg-switch.js` 的 `afterPaint`）。另外两条同源经验：量测时别指望后台标签页里的 rAF（无头环境里它几秒才回调一次，要在驱动侧用截图推帧，见 `../lab/工具/bgfade.py`）；真正影响渲染的流程（淡入淡出、过渡起始值）**必须有一次真实绘制**，光改 class 会被浏览器判成同一帧、过渡不成立 |
...
| 动态背景在 reduced-motion / 无 WebGL 的机器上**整个没了**（连静态壁纸都不见，页面是纯色） | 动态背景是 `data-bg` 的一个取值（`__shader`），而它没有图 —— 一旦 shader 起不来，选中的就是一个「既没有图、也没有 shader」的套 | 失败路径必须**换套**、不是「摘图」：`bg-shader.js` 的 `standDown()` 把 `data-bg` 从 `__shader` 换回静态兜底套（`__bgOpts.fallback`），壁纸随之回来。「选中 `__shader` 就不下发壁纸」写在**生成的 CSS** 里（`bg-shader.js` 落地时用的 `__bgNoStatic()` 行内摘图方案已被它取代 —— 行内方案晚于 CSS 一整步，首屏那几十毫秒里图已经在下了）。验：`lab/结果/bg-shader-contrast/check-degrade.py <url>` 三种情况各起一个 Edge，判 `data-bg` 是否被换回 `girl`、`--bg-image-light` 是否还是 `url(...)`。见 [`features.md` 第 3 节 ⑫](features.md) |
| 换 shader 背景后一批正文的对比度**悄悄**从 4.8~4.9 掉到 4.30（AA 线 4.5），肉眼只是「看着还行」 | 深色极光只有一套配色时，浅色主题（深字）是风险面；而壁纸套的蒙版中档只有 0.02，等于文字底下没有可依赖的底 | 配色按 `data-theme` 分两套（浅色 = 亮底白昼天光，极光只朝提亮方向叠加）。判据不能只看截图：`capture-contrast.py` + `analyze-contrast.py`，逐帧取**底色众数**、并**排除笔画像素** —— 不隐藏文字时「最坏像素」恒等于文字自己（1.00），是个假信号 |
...
| 新风格在**浅色画上不见了**（或深色画上不见了），但换一张画就正常 | 混合模式与画作明暗的相互作用：`screen` 是「提亮」，压在本来就接近白的画上等于什么都没做（金继第一版的金色裂缝放在「木刀」那张浅画上完全看不见）；`multiply` 是「压暗」，压在深色画上同理。而首页这 32 张卡里浅色画占多数 | 加「亮材料」（金、霜、雪、光）优先用 `normal` / `soft-light` / `overlay` + 半透明颜色；真要 `screen` 就把对比图做成「**同一批风格 × 浅色画 + 深色画**」两组一起看（`../lab/结果/deck2/styles-final-v2-*.png` 就是这么出的），别只挑一张画试 |
...
| 无头浏览器里 **WebGL 页面截出来一片黑、控制台却干净** | `--headless=new` 下 Chromium 从 2024 起默认不提供 GPU 后端、也不再静默回退到软件渲染，`canvas.getContext('webgl2')` 直接返回 null。而「黑屏」与「画面本来就是黑的」在截图上看不出来 —— 拿这种图做验收等于假通过。**2026-09-21 复测更正**：本机 Edge 的 `--headless=new` 已经能拿到真 GPU（`WEBGL_debug_renderer_info` 读出 `ANGLE (NVIDIA GeForce RTX 5060 Laptop GPU, Direct3D11)`），所以「headless 一定黑屏／一定软渲染」不再成立 —— 但**结论没变**：截图前仍要先读一次 RENDERER 字符串，它随 Edge 版本与机器而变，读到 SwiftShader 就别拿那张图判断画质 | `lab/工具/shots.py` 加 `--gl swiftshader\|gpu`（默认软渲染保证可比，关键观感另拍一组真显卡对照），并先跑探针页 `lab/工具/gl-probe.html` 确认拿到了上下文与真实 RENDERER 字符串。**判据不能只看截图**：探针用 readPixels 数颜色种类（软/硬渲染实测 1612 vs 1599 种） |
...
| 拆包/挪文件之后某个页面少了一段样式，**构建与所有校验全绿**，只在浏览器里看得出来 | 按页拆包的接线判据是「这一页真的渲染出哪些类」，而类是模板/JS 生成的：新加一个渲染卡片的页面时，`extend_head.html` 里 `.IsHome or .Section == "collection"` 那个条件不会自己跟上 | 由 `scripts/check-css-split.mjs`（阻断，CI / `push-blog.sh` / 体检面板三处）拿产物对账「谁带 `home-card*` / `deck-*` 就得带 `decks` 表、其余页面一张都不许多带」。**判断「哪个类属于哪一组」别用「被移出文件里的类名全集」**：`00-theme.css` 的玻璃底衬清单里也有一条 `.home-hero-subtitle`，拿它当判据会把全站判成「该加载首页表」—— 判据要选**只在被移出文件里定义**的选择器（`.home-card` / `.deck-` / `.deck-splash`，都先核过留下的文件里一个都没有）|
| 想拿「没有视觉回归」的证据，截图对拍却总在几个像素上不一致，看着像改坏了 | 页面有动画（时钟、入场淡入）与 WebGL 台面，截图会被时序污染；而**逐节点比计算样式**也会被两件事骗：① 新表多出的 `<link>` 让 `<head>` 的子索引整体位移，按 DOM 路径对齐就变成拿 A 元素比 B 元素（实测把 166 个节点全报成「差异」）；② 时钟类节点每秒都在变 | 用 `../lab/工具/stylesig.py`（逐节点 `getComputedStyle` + 先注入 `animation:none;transition:none` 冻结动效）：**先跑一次「同一产物 vs 自己」定噪声基线**（实测首页 3 个节点、`/collection/` 0 个），再跟改后比；对齐时排除 `head` 子树（路径以 `1.` 开头），改成按「tag+class 的文档序第 n 个」编键。2026-09-23 拆主样式时用它拿到「首页 341 / 收藏库 1267 / 列表页 166 / M1 notes 26871 个节点，真差异 0」，顺带查出根上少掉的 28 个自定义属性（`--cardback-*` / `--deckstage-*` / `--tex-*` / `--mark-*` / `--fret-angle`）—— 再核一步「谁读它们」：只有 `card-3d.js`，而它只在首页与收藏库加载，没有孤儿 |
...
| 模板里改了文案，页面上那一块**变成空白**（不报错） | `i18n "键"` 拼错/改名/删键时 Hugo **不报错**：缺失键静默渲染成空串、构建全绿。卡组那几个模板一直有绑定守卫（check-deck.mjs），baseof / 404 / extend_head 等处没有 | `scripts/check-i18n.mjs`（阻断，CI / push-blog / 体检面板三处）扫 `layouts/**` 的字面量键，对 `i18n/zh.toml` ∪ 主题译文查存在与非空；动态键（`i18n (index …)`）只列出不判，反向不查（键可能被主题模板用、或故意覆盖主题译文）。缺键与空值两种注入都验过会报（给到 file:line） |
...
| 与基线逐页比产物，比出十几处「非卡片页却不同」的**假回归** | 两种成因，第二种更隐蔽：① 这台机器 `core.autocrlf` 生效，检出时把 LF 换成 CRLF，这些字节会**原样进产物**（静态文件，以及正文里内嵌的 KaTeX `annotation` —— 它就是源文本本身）；② CRLF 进了数据后又被 JSON 转义成**字面的** `\r`（搜索索引 `index.json`），**按行尾归一化盖不住它**。另外这个站点有一页会把**构建机的绝对路径**印在页面上（项目索引的文档路径），换目录构建必然不同 | 基线必须 **`git -c core.autocrlf=false clone`** 再构建（不是 `git stash` —— 那会动工作区、把 21 个文件的改动全卷进去）。比较脚本在做行尾归一化的同时要把绝对路径也归一化，并**打印归一化的处数**（免得它悄悄掩盖了真改动）。工具 `lab/结果/deck6/regress.py`，判据是「非卡片页 0 处不同」 |
| 量「名牌条里那个徽章有多大」，量出 40×30（等于窗口尺寸），看着像「徽章很大」 | 名牌条**自己也内缩**（`right: var(--cart-inset)`，低档 14px），所以按**卡右边缘**取窗口时右半边落在亮金属框上；把窗口缩到贴住卡右边缘，又会够到卡号文字（也是亮字）。同一个坑踩了两次 | **几何用像素量、元素用 DOM 量**：徽章干脆不在像素上量，改由 `lab/结果/deck6/probe.js` 读 `getBoundingClientRect` 与 `clip-path`（顺带证明了「形状真画出来了」）。非要像素量，就先 `crop` 出来看一眼那个窗口里到底有什么 —— 别从坐标算术上推 |
...
### 混合模式要在**真实底色**上验，别在色块上验

新加四种卡面工艺时，四种都按「在色块上试很好看」的直觉选了混合模式：珠母/绸缎用 `soft-light`、极光用 `screen`。
到 272px 的真卡上，四种**几乎看不出区别**，全都像「蒙了一层白雾的普通纸」。两个原因叠在一起：

1. **`soft-light` 与 `screen` 在浅色底上趋于恒等**。`screen` 加亮到白就是白 —— 极光的绿紫叠在白卡面上等于没画；
   `soft-light` 在近白底上也只剩一层很弱的提亮。这两种模式对**深色底**才有效。
2. **还有一道 `--rank-craft` 倍率**（等级的工艺强度）在乘它：收藏档 ×0.35、珍稀 ×0.55。
   覆盖层自己的 `--coverlay-op` 写着 .9 也不代表画面上是 .9 —— **实际不透明度 = op × craft × 该档系数**。

**判据**：任何「覆盖层 / 混合模式」的改动，验收时必须落在**真实卡面**上，而且**重点看低档那几张**
（低档是把差异放大的地方；高档看着还行，往往只是因为它的 craft 倍率把强度补回来了）。
本次是靠在卡片墙上按页面坐标裁单卡放大才发现的 —— `check-deck.mjs` 只核「工艺有没有声明 `--fret-line`」，管不了强度。

### 等级令牌的单调性没人替你检查

六档边框的总宽是 `--rank-frame + max(--rank-mat, --rank-band)` 算出来的，两条令牌此消彼长。
2026-09-21 量 `getComputedStyle` 才发现**史诗档 10px 比珍稀档 12px 还窄** —— 顺序倒挂，
而页面上不报错、`check-deck.mjs` 也不核这一条（它核的是「六档令牌齐不齐」，不是「六档排好序没有」）。

**判据**：六档这种「靠看起来分级」的设计，新增或调整任何一档之后，把六档的值**实测打出来排一遍序**
（`getComputedStyle(el).getPropertyValue('--rank-xxx')`），别信注释里的数字 ——
注释写的是「史诗 16px」（那是更早一版的口径），而那时实际算出来是 10px，两处对不上已经很久。
...
**验证办法**（`lab/工具/deckcheck.py`）：点一次换卡后**同一 tick** 读四点 —— ① 触发类在不在；② `getAnimations()` 里三条动画的名字与时长；③ 主图与残影的 computed opacity 起点是不是 `0 / 1`（互补）；④ 整卡的 opacity 是不是恒 1。截图看不出这些，而且 headless 里 `Page.captureScreenshot` 一次要几百 ms，等它回来 340ms 的动画早跑完了（实测每个采样点都落在结束态）—— 采样只能用纯 `evaluate` 快采。
...
- **模板里 `jsonify` 一个对象 ≠ 页面里得到一个对象**（2026-09-22 晚，动态背景的套配置踩到）：在 `<script>` 上下文里，html/template 会把 `jsonify` 的结果**再转义一次**，输出的是字符串（`sets:'{"a":1}'`）。模板不报错、构建成功，运行期 `SETS[id]` 恒 undefined —— 表现是「整套动态背景没挂」，而页面看着只是「没有背景」，最容易归因到 WebGL 或缓存上。修法是别在脚本里嵌 JSON：走 `data-opts="{...}"`（属性值由 HTML 解析器解码，没有这层转义），脚本用 `document.currentScript.dataset.opts` 读 —— 与 `data-presets` 同一招。判据：grep 构建产物确认是 `data-opts='{"…"}'` 而不是 `'{…}'`
- **TOML 里写在 `[[数组表]]` 之后的键，属于那张表的最后一个元素**（同一天踩到）：`dynamicDefault` 一度写在 `[[params.appearance.presets]]` 那几段后面，于是它落进**最后一套 preset** 而不是 `[params.appearance]` —— 模板读到空串，「首屏默认给动态套」这件事**静默地没发生**（构建不报错、页面也没坏）。绝对路径的 `[[params.appearance.shaders]]` 不受位置影响，反而容易让人以为「同一块的键都一样」。判据：`hugo` 之后去产物里 grep 那个值有没有落地，别只看配置文件写没写
...
- **动态背景引擎是惰性挂载的，`window.__bg.ok` 是「乐观」的**（2026-09-22 晚第四版）：静态壁纸的访客加载后 **DOM 里没有 `#__bgshader`、也没有 GL 上下文**（这正是省下来的那一笔：`getContext` 首次 28 ms、其后 8~15 ms，另加一块显存），要等 `data-bg` 本来就是动态套、或访客够到动态按钮（悬停/聚焦/按下 → `prepare()`）才挂。所以 `ok === true` 只表示「还没发现跑不了」（只有 reduced-motion 能在加载前判死）；跑到「无 WebGL」才知道，那一刻 `bg-shader.js` 把 `data-bg` 换回静态套并派发 `bg:dyn-unavailable`，切换脚本据此撤掉按钮。写探针时别把「静态访客没有 canvas」当故障，也别拿 `ok` 当「WebGL 可用」的判据 —— 要判就得看 `stats().mounted` / `canvas` 是否真在 DOM 里，或者 `--disable-webgl` 下走一遍悬停（`check-degrade.py` 的 ④/④′）。
- **意图预热要的是「提前量 ≥ 下载时间」，而触屏没有提前量**（同一处，`bg-switch.js` 的 `warm*Intent`）：动态开着时空闲期不再预取壁纸（跨轴那两张 164 KB 浅色 / 271 KB 深色），改由悬停/聚焦/按下触发 —— 悬停到点击之间那段时间就是它唯一的本钱。深色那两张各 ~137/128 KB，512 kbit/s 下要 ~3.5 s，实测 **2.5 s 的提前量不够**（读数与「完全不预热」几乎一样，都是 875 ms 顶到 `DECODE_MAX_MS` 上限）。所以调这几个数时要照最坏情况（触屏、慢网）看，别只量快网鼠标那一档。
...
- **`lab/结果/` 这套脚本有三处 CDP 坑，第三轮的 hover 探针一次踩全**（2026-09-21，脚本见 `../lab/结果/deck8/{hover,zoom_filters}.py`）：
...
- **safebooru 的 dapi 有两个反直觉行为**（2026-09-18 抓「黑长直少女」背景素材时踩到，脚本留在 `../lab/工具/pick-girl-bg.py`）：① **无结果时返回的是空响应体，不是 `[]`**，`json.loads('')` 直接抛异常——看着像网络坏了，其实是那个标签组合真的 0 条（`1girl+solo+…+wide_image` 就是这种）；② `rating:general` 只是 `rating:safe` 的一个**小子集**（同一组标签实测 95 vs 437），拿 `general` 当「SFW 全集」会白白丢掉四分之三的候选。另外 `sample_url` 的长边被压到 1500，**不能当成品源**（成品要 1600 宽，会放大），它只适合拼接触表；选中的要按 `file_url` 下原图。`pic.re` 的 `file_url` 则相反：没有协议前缀（`cdn.pic.re/…`），直接喂 `urllib` 报 `unknown url type`，要自己补 `https://`
- **别在同一个输出目录上并发跑两次抓图脚本**：`../lab/工具/pick-girl-bg.py` 一次要跑上百个请求，重复启动会让两边同时写同一批文件（实测出现过「元数据写成了 `[]`，但样本图还在陆续落盘」的错位状态，看不出以哪次为准）。脚本里对已存在的图有短路复用（`get_to`），所以重跑很便宜——先确认没有残留进程再跑
- **性能量法脚本的路径可能与本机不一致**：2026-09-19 起量法脚本统一在仓库外的 `../lab/结果/`（此前 `.shots/` 与本仓库并列，`features.md` 里旧写的 `D:\projects\blog\lab\结果\` 并不存在）；但 `startjank.py` **内部**还有两处写死的 `D:\projects\blog\lab\shots`（Edge 的 profile 目录与输出 json），直接跑会报错。复跑时把脚本连同 `frameab.py` 复制到临时目录、只替换那两处路径，**别改原文件**（路径是别人机器上的布局）。依赖：Edge + `websockets` + `python` 都在本机可用
...
- **「接入 Lighthouse CI」收益低于成本**：站点已有一整套校验（含 `report-size.sh` 的单页/整站/索引体积预算）与每周 lychee 外链检查；Lighthouse CI 在 Pages 子路径下需要额外的 server 与 token，且它的产出与本地 `../lab/结果/` 那套手测量法重复
