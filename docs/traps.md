...
| 动态背景在 reduced-motion / 无 WebGL 的机器上**整个没了**（连静态壁纸都不见，页面是纯色） | 摘静态壁纸（把 `--bg-image-*` 置 `none`）写在了预置脚本里 —— 预置脚本一定执行，而挂载 shader 可能早退，于是「图摘了、shader 没起」 | 摘图**只能在 GL 上下文与着色器都成功之后**（由 `bg-shader.js` 自己调 `__bgNoStatic()`）。三条早退路径（reduced-motion / 无 WebGL / GLSL 编译失败）都必须留着原图。验：`lab/shots/bg-shader-contrast/check-degrade.py <url>` 三种情况各起一个 Edge，判 `__bg.why` 与 `--bg-image-light` 还是不是 `url(...)`。见 [`features.md` 第 3 节 ⑫](features.md) |
