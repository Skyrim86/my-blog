...
显示文字的顺序：显式第二参数 → 卡片名字 → 卡片自己的 `label`（「定理 10.2」）→ id。点开由 `assets/js/toolbox.js` 拦成弹窗，**没有 JS 时就是普通链接**，跳到完整可读的卡片页。弹窗是**多栏**的：在卡片里再点一张（正文引用、或卡片底部的「配件 / 用在哪」），那一张在旁边新开一栏，原来那张留在左边；栏数不设上限，多了横向滚动（窄屏改上下堆叠）。`Esc` 一层一层退：先收最后开的那栏，只剩一栏时关掉整个弹窗。

**主次关系：配件挂在正主下面**（`parents` 字段，2026-09-19 起）

引理、推论、性质是**配件** —— 它们本来就不该跟定理、定义平起平坐地各占一个卡位（引理是给人用的工具，推论是顺着定理下来的，性质是某个东西的性质）。所以卡片墙上它们挂在正主那一格里：正主卡**右侧有一个折叠箭头**（可访问名是「N 个配件」），**默认收起**，点开才在正主下面铺开那几张小一号的卡。谁挂谁由一个 `parents` 字段决定（字段里是正主的 id 列表；一张配件可以挂好几家）。

**反过来的方向也在页面上**：每张卡**自己**的底部有一行关系列表（`_partials/card-rel.html`）—— 正主列出「配件」，配件列出「用在哪」。所以「引理 4.1 服务哪几个定理」在它自己的卡片页上就能看全，不必回墙上数那几份缩进的重复份（重复份只是**墙上的显示口径**，不是数据）。

- **谁算配件**：`引理` / `推论` / `性质`。别的类别（定义、概念、定理、命题、公式…）都是正主。
- **只挂两层**：正主不挂别人，配件也不再挂配件（否则墙面变成三层缩进）。`scripts/check-cards.mjs` 会拦住「挂到配件下面」。
...
- **手写表 `data/card-parents.yaml`**（不是生成产物，与 `math-branches.yaml` 同待遇）：只该出现在规则推不出来的地方，当前是那三张公用引理。表里有条目就用表里的（支持一条挂多个正主），写空列表 = 明确不挂。**表里的 `why` 要写清依据** —— 下次改的时候不用重读笔记。
  - 为什么引理必须手写：这层关系是「谁用了谁」，而正文里查不到（数学卡正文那 48 处交叉引用全是引工具卡，没有一处引引理），只能在读笔记时逐条点明白。
- **CS 库**的卡片 JSON 是**手写维护**的，所以那边没有推导规则：要挂就在 `data/cs-toolbox.json` 里给那张卡直接写 `"parents": ["正主 id"]`。当前 CS 的三张性质各自独占一节（1.1、6.1、7.1），前后没有正主，就没写 —— 它们照旧单独显示。
- 墙上的渲染（折叠箭头、重复份）见 [`features.md` ㉑](features.md)，卡片自己的关系列表见 `_partials/card-rel.html`；改这张表或这个字段后跑 `node scripts/check-cards.mjs`（CI 阻断项，查 id 是否存在、是否自指、是否成环、是否挂到了配件下面）。
