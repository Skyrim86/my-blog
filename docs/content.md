...
引理、推论、性质是**配件** —— 它们本来就不该跟定理、定义平起平坐地各占一个卡位（引理是给人用的工具，推论是顺着定理下来的，性质是某个东西的性质）。所以卡片墙上它们**缩进排在正主下面**，由一个 `parents` 字段决定挂到谁（字段里是正主的 id 列表；一张配件可以挂好几家）。
...
- 墙上的渲染见 [`features.md` ㉑](features.md)；改这张表或这个字段后跑 `node scripts/check-cards.mjs`（CI 阻断项，查 id 是否存在、是否自指、是否成环、是否挂到了配件下面）。
