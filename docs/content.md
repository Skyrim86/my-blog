...
一条笔记太长时按 § 拆成多页：生成脚本只建脚手架、建到已存在的路径会拒绝（`new-content.sh` 的「已存在，拒绝覆盖」），所以**拆页是手工活**，切点按 § 的整边界走。1575 行、2500 多个数学区渲染出来约 2.5 MB，会撞 `report-size.sh` 的单页预算。

项目文档页（`content/projects/<项目>/<子项目>/*.md`）同样受单页预算约束，但没有导入管线可用，得手工拆：把附录这类**不参与正文推导**的整块（伪代码、代码清单）搬成同目录下的一页（先例：问题三的附录 A/B → 《问题三_参考实现》，`math: false` 关掉 KaTeX，主页附录处留一行指向新页）。手工拆的**唯一代价是锚点**——外部书签指向 `问题三/#附录-b…` 会失效，站内引用则要跟着改（grep `问题三.md》` 之类）。**课程笔记同款先例**（2026-09-26）：chapter-01 三重页按 § 二分为（一）–（五），不依赖 `<pre>` 大块——KaTeX 树型的重页直接按 § 切即可（pending「M1 三页拆页」有全部切点与实测数字）。
...
**主次关系：附属结论挂在主卡下面**（`parents` 字段，2026-09-19 起）

引理、推论、性质是**附属结论** —— 它们本来就不该跟定理、定义平起平坐地各占一个卡位（引理是给人用的工具，推论是顺着定理下来的，性质是某个东西的性质）。所以卡片墙上它们挂在主卡那一格里：主卡**右侧有一个折叠箭头**（可访问名是「N 个附属结论」），**默认收起**，点开才在主卡下面铺开那几张小一号的卡。谁挂谁由一个 `parents` 字段决定（字段里是主卡的 id 列表；一张附属结论可以挂好几家）。

**反过来的方向也在页面上**：每张卡**自己**的底部有一行关系列表（`_partials/card-rel.html`）—— 主卡列出「附属结论」，附属结论列出「所属」。所以「引理 4.1 服务哪几个定理」在它自己的卡片页上就能看全，不必回墙上数那几份缩进的重复份（重复份只是**墙上的显示口径**，不是数据）。

- **谁算附属结论**：`引理` / `推论` / `性质`。别的类别（定义、概念、定理、命题、公式…）都是主卡。
- **只挂两层**：主卡不挂别人，附属结论也不再挂附属结论（否则墙面变成三层缩进）。`scripts/check-cards.mjs` 会拦住「挂到附属结论下面」。
- **规则**（没在表里写明的卡按这个推，`import_course.py` 与 `gen-cards.mjs` 两侧同一套判据）：
  - `推论` → 同一节里排在它前面、最近的 `定理` / `命题`；
  - `性质` → 同一节里排在它前面的最近的 `定义` / `概念`（性质是「某个东西的性质」）；
  - `引理` → 同样退回「前面最近的定理/命题」，但引理常常一条服务好几个定理，所以三张都写进了手写表；
  - 「同一节」按 **id 前缀 + 编号第一段**分组，工具卡（`tool-2-x`）与课程笔记卡（`thm-2-x`）虽然数字撞上，但是两节，不会互相挂。
- **手写表 `data/card-parents.yaml`**（不是生成产物，与 `math-branches.yaml` 同待遇）：只该出现在规则推不出来的地方，当前是那三张公用引理。表里有条目就用表里的（支持一条挂多个主卡），写空列表 = 明确不挂。**表里的 `why` 要写清依据** —— 下次改的时候不用重读笔记。
  - 为什么引理必须手写：这层关系是「谁用了谁」，而正文里查不到（数学卡正文那 48 处交叉引用全是引工具卡，没有一处引引理），只能在读笔记时逐条点明白。
- **CS 库**的卡片 JSON 是**手写维护**的，所以那边没有推导规则：要挂就在 `data/cs-toolbox.json` 里给那张卡直接写 `"parents": ["主卡 id"]`。当前 CS 的三张性质各自独占一节（1.1、6.1、7.1），前后没有主卡，就没写 —— 它们照旧单独显示。
- 墙上的渲染（折叠箭头、重复份）见 [`features.md` ㉑](features.md)，卡片自己的关系列表见 `_partials/card-rel.html`；改这张表或这个字段后跑 `node scripts/check-cards.mjs`（CI 阻断项，查 id 是否存在、是否自指、是否成环、是否挂到了附属结论下面）。
