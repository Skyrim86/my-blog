...
CASES = [
    ('m1 手改生成物（--coverlay-op 1 → .9）',
     'assets/css/decks/21-card-styles.css', '--coverlay-op: 1;', '--coverlay-op: .9;'),
    ...
    ('m3 把手写块加回来（.home-card--silk 放回 21-card-deck.css）',
     'assets/css/decks/21-card-deck.css',
     '/* 极光：**高层大气发光**',
     '.home-card--silk {' + CRLF + '  --fret-line: #fff;' + CRLF + '}' + CRLF + CRLF + '/* 极光：**高层大气发光**'),
    ('m4 生成物里丢掉 --fret-line（silk）',
     'assets/css/decks/21-card-styles.css', '  --fret-line: rgba(255, 246, 240, .92);', '  /* 删掉这一行 */'),
