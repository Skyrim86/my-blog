...
//         ⚠ 的常见来源是**匿名配额 60/小时用光 → HTTP 403**（连着复核几次就会撞上）。
//         那时不要拿旧 run 或 workflow 徽章凑一个「绿」——徽章不带 SHA，会把上一次的成功
//         算到这一次头上。立刻要确定答案：等一小时，或者用**内容级**核对
//         （线上页面与本地产物的 sha256/指纹逐字节比，例如深度图 URL 里那串哈希）。
//
