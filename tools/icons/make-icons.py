...
# 五官会糊成一团、边缘还会留一圈灰毛边（实测对比留在 ../lab/shots/navtest/sweep-emilia.png）。
...
NAV_CHARS = [
    ...
    # 收藏库（2026-09-21 加）：从**卡片组自己的五个系列**里挑一个当门脸。
    # 挑初音未来而不是张数最多的艾米莉亚（27 张，她已经站在「关于」那一项上）的理由是**色相**：
    # 16px 下认人靠发色，而现有八人的发色里没有青绿（奶白/白绿/淡蓝紫/天蓝/金/暖棕/深紫/银白），
    # 青绿在这条色带上是孤点；剪影靠 twins 那两支垂到画面底的马尾。
    # 配饰是新建的 ribbon（缎带蝴蝶结，见 draw_nav_chibi）：八个配饰剪影里没有它，洋红压青绿是互补色。
    ("collection", "初音未来", dict(
        hair=((78, 202, 190), (150, 232, 222), (36, 142, 142)),
        eye=((52, 160, 196), (26, 98, 132)), cloth=((44, 46, 58), (78, 202, 190)),
        style="twins", acc="ribbon", acc_c=(228, 92, 132))),
    ("library", "纳西妲", dict(
...
def draw_nav_chibi(spec):
    ...
    if acc == "crown":                 # 派蒙：三尖小金冠（第一版只画了一条线，16px 下看不见）
        ...
        for y in range(11, 15):
            c.set(3, y, rc)
    elif acc == "ribbon":              # 初音未来：左上的缎带蝴蝶结（两瓣 3×3 + 浅色结）。
        # 第一版是 2×2 的两瓣、只有一行相连 —— 32px 下只是一个粉色小块，读不出「蝴蝶结」。
        # 现在照 `plum` 那一枚的占地（x3~9 / y6~8）：两瓣各 3×3、中间一列是浅色的结，
        # 三行都连通所以不会散成两块。位置在刘海之上、头顶轮廓之内，不会被头发吃掉。
        rc = spec["acc_c"]
        for y in (6, 7, 8):
