...
MAX_TOTAL_KB="${MAX_TOTAL_KB:-27648}" # 整站输出上限 27 MB（2026-09-21 与上面那条一起抬：多了 63 张 760px 卡面，raw 也涨约 2.4 MB）（2026-09-15：新增「回归分析」课程 —— 拆成 3 页的 M1 笔记 + 作业 + 实验，以及 80 张数学工具卡页（一卡一页，见 tools/course-import/），整站 10.0 → 19.0 MB；单页预算不变）
# 2026-09-21 从 52 抬到 56：**不是哪一条变胖，是内容真变多了**。逐条量过：27 个条目、每条约 1.8 KB
# （正文 22.6 + 页内标题 12.4 + 其余 13.0 KB），最重的一条 2.7 KB —— 把每条正文截到 1000 字节
# 也只能省 0.3 KB（正文本来就已经很短了）。**真正的风险是「再加两页就顶到」**：条目数是线性增长，
# 所以这条预算要盯着条数，而不是盯某一条。访客视角：搜索索引是打开搜索时才下的那个文件，
# 压缩后约 13 KB（JSON 压得很好），所以抬这 4 KB 对首屏没有影响。
MAX_INDEX_KB="${MAX_INDEX_KB:-56}"   # 搜索索引上限 56 KB（含页内标题 headings；条目数是线性增长的）
...
#
# 2026-09-20 晚再抬到 6656（6.25 → 6.5 MB）：同一天又加了两个角色系列（艾米莉亚 27 张、
# 初音未来 4 张）。**这一档的账**：31 张 × (卡面约 44 KB + 深度图约 5 KB) ≈ 1.5 MB，
# 正好是 4563 → 6358 KB 的全部增量（差额约 100 KB 来自取景口径改动：卡面从「补边留白」
# 改成「填满」，画面细节变多、压缩率略降）。实测 6358 KB，抬到 6656 留约 300 KB 余量。
# 仍然遵守同一条原则：**不靠压画质硬凑**，每一张卡都是访客会一张张转着看的东西。
#
# 2026-09-21 抬到 9600（6.5 → 9.4 MB），raw 抬到 27648：3D 查看器的台面从 430px 放大到 600px，
# 卡面贴图从 544px 那档撑不住了，于是新出**第三档 760px（q72）**，只给查看器与弹层大图用。
# **逐目录对过账**（对 HEAD 各做一次新构建）：整站 +2455 KB raw / +2439 KB gzip，其中
#   · +2441 KB = 63 张 760px 卡面（每张约 39 KB；webp 已压过，gzip 与 raw 几乎相同）——**唯一的大头**
#   · +6.2 KB = card-3d.js（新通道、余韵、六档阶梯），另外两个页面的 HTML 各 +3.9 KB（清单多一个字段）
#   · −14 KB = 深度图重出后**更小**（倒角把断崖变成圆坡，压缩率反而更好）
# 访客视角不变：760px 那档**不进 srcset**，列表页一个字节都不多下，只有打开弹层时才多下一张
# （约 39 KB，与今天下 544px 那档相差约 12 KB）。抬到 9600 留约 690 KB 余量。
# 2026-09-22 抬到 11264（9.4 → 11 MB）：收藏库卡片墙加了一档 412px（63 张 × 20.4 KB ≈ 1286 KB，
# 就是这次的全部增量 9222 → 10512）。**这是一笔「用磁盘换访客带宽」的交易**：DPR2 下收藏库一页的
# 卡片下载从 1710 KB 降到 1286 KB（−24.8%），DPR1 基本不变（只省下 --art 那一份，−36 KB）；
# 代价是多 1.29 MB 产物 —— 前者每个访客都付，后者只在仓库/部署里占一次。实测与观感见
# docs/features.md 追加九。**不把 544 档换掉**：3× 手机与 760px 弹层还要它。
MAX_COMP_TOTAL_KB="${MAX_COMP_TOTAL_KB:-11264}" # 整站 gzip 上限 11 MB
