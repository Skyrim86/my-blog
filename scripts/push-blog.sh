...
if [ -n "$dirty" ]; then
  ...
  # 卡片数据不变量：**阻断**。每张卡都要有名字（外加 id/kind 非空、id 不重复）——
  # 无名卡在卡片墙上只是一个裸类别词（「引理 4.1」），等于没有名字。数学卡那边由课程项目的
  # md 决定，本地推送时课程项目就在机器上，但这条查的是**产物**，所以推导出的 JSON 脏了
  # （手改过、知识库发布、管理页改过）一样会被拦。
  if [ -f scripts/check-cards.mjs ]; then
    echo "▸ 卡片数据校验（每张卡都要有名字）"
    if cc_log="$(node scripts/check-cards.mjs 2>&1)"; then
      printf '%s\n' "$cc_log" | tail -1 | sed 's/^/  /'
    else
      printf '%s\n' "$cc_log" | sed 's/^/  /'
      echo "✗ 有卡片缺名字，已中止（未提交、未推送）。"
      echo "  （数学卡的名字改课程项目的 md 再重跑导入；CS 卡补 data/cs-toolbox.json。）"
      exit 1
    fi
  fi
