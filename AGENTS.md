...
| 首页 hero 的角色立绘（绫华，透明底抠图） | `_partials/home-chara.html`、`09-home.css`、`hugo.toml` 的 `[params.home].chara`、`tools/backgrounds/make-ayaka-chibi.py`（**生成产物，不要手改 assets/images/ayaka-chibi.webp**） | 同上 ㉕（**抠法刻意与看板娘不同**：那张身上有纯白，必须用边缘灌水；理由与实测数字都在那一节） |
