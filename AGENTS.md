...
| 首页（头像 / 快捷入口 / 站点规模 / **时钟** / 最近更新 / 入场动效 / ≥1024px 两栏） | `_partials/index_profile.html`（整份覆盖）+ `_partials/home-scale.html`（站点规模）+ `_partials/home-clock.html` + `assets/js/home-clock.js`（时钟）+ `_partials/home-chara.html`（角色立绘）、`09-home.css`、`hugo.toml` 的 `[params.home]`。**「按类浏览」那一栏 2026-09-21 已删**（`home-extras.html` 连 `09-home.css` 里的 `.home-browse` 组、4 个 i18n 键一起删掉了 —— 首页因此没有通往 `/categories/` 的入口，只剩主导航与 `/tags/`） | [`docs/features.md`](docs/features.md) ⑮ 与 ㊼ |
