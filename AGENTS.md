...
| 站点背景（**两根轴**：静态壁纸 = 黑长直少女 / 城市；动态背景 = WebGL 极光 / 夜樱）+ 每套的蒙版 + 压在壁纸上的**局部玻璃底衬**（含页脚整条）与玻璃质感层 | `hugo.toml` 的 `[params.appearance].presets`（静态）与 `.shaders` + `dynamicDefault`（动态，可多套）、`layouts/_partials/extend_head.html`（生成 `css/bg-image.css` + 首屏前置脚本 + shader 线）、`assets/js/bg-shader.js`（**动态背景本体**：多套引擎，`KINDS` 一个键一种画法；按 `data-theme` 切两套配色；三条降级路径写在文件头）、`assets/js/bg-switch.js`（**两个按钮**：壁纸 / 动态背景，各存各的偏好，耦合只有「点壁纸时先关动态」；按钮形状与悬停 2026-09-19 起在 `10-nav.css` 的「顶栏图标按钮」一节，`18-bg-switch.css` 已只剩说明）、`00-theme.css` 的 `body::before`（蒙版）与 `body::after`（质感层）及末尾的底衬清单、`tools/backgrounds/make-backgrounds.py`（**生成产物，不要手改 assets/images/bg-*.webp**） | [`docs/features.md`](docs/features.md) 第 3 节 ⑫（两根轴 / 两个按钮 / 四套的对比度实测 / 判据变更 / 复跑清单）+ [`docs/exp-bg.md`](docs/exp-bg.md)（代价与帧间隔、夜樱的逐版返工记录） |
