...
| 玻璃透镜（面板与壁纸的真折射） | `assets/css/extended/20-lens.css`、`layouts/_partials/lens-defs.html`（SVG 滤镜，由 `baseof.html` 插在 `<body>` 开头，见规则 5）、`assets/css/extended/00-theme.css` 的 `body::before`（只加滤镜与晕影） | [`docs/features.md`](docs/features.md) 第 3 节 ⑫ 末段（含对齐/对比度/合成开销三组实测） |
...
| 站点外观（配色 / 深色令牌 / 装饰层 / 透镜） | `assets/css/extended/00-theme.css`、`03`…`13` 组件样式、`20-lens.css`（透镜，见上一行） | [`docs/features.md`](docs/features.md) 第 3 节 ⑫（令牌）与 ㉔（装饰层） |
