...
| 首页的卡片组（28 张、一次一张、手动 + 自动切换；另有「hero 场景插画」一种备选形式） | `data/home-cards.yaml`（**清单，单一事实源**）、`_partials/home-cards.html`、`assets/js/home-deck.js`、`assets/css/extended/21-card-deck.css`、`tools/cards/make-cards.py` + `tools/cards/sources/`（**生成产物，不要手改 `assets/images/cards/*.webp`**）；备选形式见 `_partials/home-chara.html` + `tools/backgrounds/make-ayaka-home.py` | 同上 ㉕（六种卡面风格、取景判据、名牌对比度那个坑） |
