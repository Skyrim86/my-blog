...
6. **JS 放 `assets/js/*.js`**，由 `extend_head.html` 用 `resources.Get | minify | fingerprint` 接线外链；不要内联 `<script>`（无 lint、无压缩、内联 defer 无效）。**CSS 放 `assets/css/extended/`**，一个职责一个文件、用 `NN-` 前缀控制合并顺序；模板里不要写 `<style>`。**只服务少数页面的 CSS 不放这里**：首页 + 收藏库用的放 `assets/css/decks/`、只有首页用的放 `assets/css/home/`，由 `extend_head.html` 条件外链（`scripts/check-css-split.mjs` 拿产物对账，见 docs/features.md 第 6 节追加十）—— 这两个目录里的文件挪回 `extended/` 就等于每页又吃一遍。**唯一例外**是 `extend_head.html` 里那段背景套的首屏前置脚本（约 200 字节）：它必须在**首次绘制前**把 `data-bg` 写到 `<html>` 上，走 defer 外链会先按默认套下一张图再换成访客存的那张——白下载一百来 KB 还闪一下；主题自己处理 `pref-theme` 用的是同一招。理由与登记见 [`docs/traps.md`](docs/traps.md)
...
| 站点背景（**两根轴**：静态壁纸 = 黑长直少女 / 城市；动态背景 = WebGL 极光 / 夜樱）+ 每套的蒙版 + 压在壁纸上的**局部玻璃底衬**（含页脚整条）与玻璃质感层 | `hugo.toml` 的 `[params.appearance].presets`（静态）与 `.shaders` + `dynamicDefault`（动态，可多套）、`layouts/_partials/extend_head.html`（生成 `css/bg-image.css` + 首屏前置脚本 + shader 线）、`assets/js/bg-shader.js`（**动态背景本体**：多套引擎，`KINDS` 一个键一种画法；按 `data-theme` 切两套配色；三条降级路径写在文件头）、`assets/js/bg-switch.js`（**两个按钮**：壁纸 / 动态背景，各存各的偏好，但**同一时刻只显示一套**（互斥：点壁纸会把动态关掉，动态开着时这一下不换套；点动态则壁纸退场 —— 2026-09-22 晚第五版口径）；壁纸字节只沿当前这条轴预取，跨轴那两张改成「悬停/聚焦/按下」的意图预热，**动态开着时空闲期零壁纸下载** —— 2026-09-22 晚第四版，理由与四个读数见 `docs/features.md` 第 3 节 ⑫ 的 ③）；按钮形状与悬停 2026-09-19 起在 `10-nav.css` 的「顶栏图标按钮」一节，`18-bg-switch.css` 已只剩说明）、`00-theme.css` 的 `body::before`（蒙版）与 `body::after`（质感层）及末尾的底衬清单、`tools/backgrounds/make-backgrounds.py`（**生成产物，不要手改 assets/images/bg-*.webp**） | [`docs/features.md`](docs/features.md) 第 3 节 ⑫（两根轴 / 两个按钮 / 四套的对比度实测 / 判据变更 / 复跑清单）+ [`docs/exp-bg.md`](docs/exp-bg.md)（代价与帧间隔、夜樱的逐版返工记录） |
...
| **主样式按页拆包**（每页只吃自己用得到的 CSS） | `assets/css/{extended,decks,home}/` 三个目录、`layouts/_partials/extend_head.html`（按 `.IsHome` / `.Section` 条件外链）、`scripts/check-css-split.mjs`（阻断） | [`docs/features.md`](docs/features.md) 第 6 节**追加十** |
...
| 本地管理页（新建 / 编辑 / 发布 / 收藏库面板 / 卡片库面板 / 体检面板 / 命令面板 `Ctrl+K` / 最近打开 / 插图） | `tools/admin/`（`start.sh`/`server.mjs`/`lib/`/`ui/`；收藏库清单的写盘入口是 `scripts/deck-edit.py`） | [`docs/admin.md`](docs/admin.md) |
