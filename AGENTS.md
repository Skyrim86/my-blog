...
| 首页的卡片组（32 张、一次一张、手动 + 自动轮播 + ⤢ 弹层看大图；另有「hero 场景插画」一种备选形式） | `data/home-cards.yaml`（**清单，单一事实源**）、`_partials/home-cards.html`、`assets/js/home-deck.js`、`assets/css/extended/21-card-deck.css`、`tools/cards/make-cards.py` + `tools/cards/fetch-sources.py` + `tools/cards/sources/`（**生成产物，不要手改 `assets/images/cards/*.webp`**）、`scripts/check-deck.mjs`（清单校验，CI / push-blog / 体检面板三处都跑）；备选形式见 `_partials/home-chara.html` + `tools/backgrounds/make-ayaka-home.py` | 同上 ㉕（十四种卡面风格与四条规矩、取景判据、出处与对比度那两个坑） |
